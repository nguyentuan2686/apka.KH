...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]))
{
    ...
    else // if there;s no any errors - create user
    {
        ...
        if(count(array_filter($arParams['REGISTRATION_GROUP'])) > 0){
            $arResult['VALUES']["GROUP_ID"] = array_filter($arParams['REGISTRATION_GROUP']);
        }
...
$arResult['REGISTER_DONE'] = $register_done;
