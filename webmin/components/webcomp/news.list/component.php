...
if(isset($arResult["ID"]))
{
    ...
    if($arParams["PARENT_SECTION"] > 0){  
    $arFilterSectionCurrent = array(
		"ACTIVE" => "Y",
		"GLOBAL_ACTIVE" => "Y",
		"IBLOCK_ID" => $arResult["ID"],
		"CNT_ACTIVE" => "Y",
	);
    $arFilterSectionCurrent["ID"] = $arParams["PARENT_SECTION"];
	$rsSections = CIBlockSection::GetList(array(), $arFilterSectionCurrent, false,array("UF_*"));
    $arSection = $rsSections->GetNext();
    if($arSection['UF_SEO_KEYWORD']!=''){
        $APPLICATION->SetPageProperty("keywords", $arSection['UF_SEO_KEYWORD']);
    }
