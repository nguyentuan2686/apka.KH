var
	one_gif_src = '/webmin/images/1.gif',
	image_path = '/webmin/images/fileman/htmledit2',
	c2wait_path = image_path + '/c2waiter.gif',
	global_iconkit_path = image_path + '/_global_iconkit.gif',
	settings_page_path = '/webmin/admin/fileman_manage_settings.php?sessid=' + bxsessid,
	editor_action_path = '/webmin/admin/fileman_editor_action.php?sessid=' + bxsessid,
	editor_dialog_path = '/webmin/admin/fileman_editor_dialog.php',
	flash_preview_path = '/webmin/admin/fileman_flash_preview.php',
	manage_snippets_path = '/webmin/admin/fileman_manage_snippets.php?lang=' + BXLang + '&site=' + BXSite + '&sessid=' + bxsessid,
	dxShadowImgPath = '';
...
BXHTMLEditor.prototype.LoadComponents2 = function(oCallBack)
{
	...
	lc.Send('/webmin/admin/fileman_load_components2.php?lang='+BXLang+'&site='+BXSite+'&load_tree=Y');
};
...
BXHTMLEditor.prototype.SystemParse_ex = function(sContent)
{
	...
	if (this.limit_php_access)
	{
		...
		sContent = sContent.replace(/#PHP(\d{4})#/ig, function(s, s1){
			return "<img src=\"/webmin/images/fileman/htmledit2/php.gif\" id=\"" + _this.SetBxTag(false, {tag: 'php_disabled', params: {value: s1}}) + "\"";});
	}
	...
	if(arScripts.length > 0)
	{
		...
		for(i=0; i<arScripts.length; i++)
		{
			...
			else if(!limit_php_access)
			{
				...
				newstr += str.substr(plast, arScript[0]-plast) + '<img id="' + id + '" src="/webmin/images/fileman/htmledit2/php.gif" border="0"/>';
			}
			else
			{
				if (window.BS_MESS)
