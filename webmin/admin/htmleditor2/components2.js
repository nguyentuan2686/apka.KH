...
function BXComponents2Taskbar()
{
	...
	BXComponents2Taskbar.prototype.FetchArray = function (clear_cache)
	{
		...
		if (window.arComp2Elements)
			...
			CHttpRequest.Send('/webmin/admin/fileman_load_components2.php?lang='+BXLang+'&site='+BXSite+'&load_tree=Y'+(clear_cache === true ? '&clear_comp2_cache=Y' : ''));
	}
	...
	BXComponents2Taskbar.prototype.ShowComponentParam = function(P)
	{
		...
		if (oTaskbar.arCurrentTooltips[propertyID] && pMainObj.showTooltips4Components)
		{
			...
			oBXHint.oIcon.className = "bx-comp2-hint";
	...
	BXComponents2Taskbar.prototype.LoadComp2Params = function (arProps, calbackFunc, calbackObj, calbackParams, method, data)
	{
		...
		var url = '/webmin/admin/fileman_load_comp2_params.php?lang=' + BXLang + '&site=' + BXSite + '&cname=' + elementName+'&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : '')+"&loadhelp="+loadHelp+'&tname=' + templName ;
	...
	BXComponents2Taskbar.prototype.loadTemplateParams = function(componentName, templateName, calbackFunc, calbackObj, calbackParams, method, data)
	{
		...
		if (method == 'POST' && data)
			_CHttpRequest.Post('/webmin/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&cname='+componentName+'&tname='+templateName+'&mode=params&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : ''),data);
		else
			_CHttpRequest.Send('/webmin/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&cname='+componentName+'&tname='+templateName+'&mode=params&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : ''));
	}
...
C2Parser.prototype =
{
	...
	GetRenderedContent : function(P)
	{
		...
		var
			url = '/webmin/admin/fileman_comp2_render.php?sessid=' + bxsessid + '&site=' + BXSite,
	...
	SelectComponent : function(pEl)
	{
		...
		setTimeout(function()
		{
			...
			//SETTINGS[_this.pMainObj.name].arTaskbarSettings['BXPropertiesTaskbar'].show = false; // hack
		}, 250);
	...
	if (_oFunc.name.toUpperCase()=='$APPLICATION->INCLUDECOMPONENT')
	{
		...
		//try{
			var
				...
				icon = (comProps.icon) ? comProps.icon : '/webmin/images/fileman/htmledit2/component.gif',
...
function loadComp2TemplateLists(siteTemplate,calbackFunc, calbackParams,method,data)
{
	...
	if (method == 'POST' && data)
		_CHttpRequest.Post('/webmin/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&mode=list&stid='+siteTemplate,data);
	else
		_CHttpRequest.Send('/webmin/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&mode=list&stid='+siteTemplate);
}
