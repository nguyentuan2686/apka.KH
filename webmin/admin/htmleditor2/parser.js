...
GetHTML: function(bFormatted)
{
	//try{
		...
		if (id)
		{
			bxTag = this.pParser.pMainObj.GetBxTag(this.arAttributes["id"]);
			if(bxTag.tag)
			{
				switch(bxTag.tag)
				{
					case 'img':
						...
						if (w && !isNaN(w))
							bxTag.params.width = w + "px";
						if (h && !isNaN(h))
							bxTag.params.height = h + "px"
...
SystemParse: function(sContent)
{
	...
	// Link
	sContent = sContent.replace(/(<noindex>)*?<a([\s\S]*?(?:.*?[^\?]{1})??)(>[\s\S]*?<\/a>)(<\/noindex>)*/ig,
	...
	sContent = sContent.replace(/<script[\s\S]*?\/script>/gi, function(sContent){return _this.UnparseScripts(sContent);});
	sContent = sContent.replace(/<!--[\s\S]*?-->/gi, function(sContent){return _this.UnparseComments(sContent);});

	sContent = sContent.replace(/<a(\s[\s\S]*?)(?:>\s*?<\/a)?(?:\/?)?>/ig, function(sContent){return _this.UnparseAnchors(sContent);});
...
{
	var id = this.pMainObj.SetBxTag(false, {tag: "script", params: {value : sContent}});
	return '<img id="' + id + '" src="' + image_path + '/script.gif" />';
},

UnparseComments: function(sContent)
{
	var id = this.pMainObj.SetBxTag(false, {tag: "comments", params: {value : sContent}});
	return '<img id="' + id + '" src="' + image_path + '/comments.gif" />';
},
