...
BXFMSearch.prototype = {
	...
	Request : function(action, postParams, callBack, bShowWaitWin)
	{
		...
		var actionUrl = '/webmin/admin/fileman_admin.php?lang=' + this.lang + "&fu_action=" + action + "&site=" + this.site + "&" + this.sessid_get;
...
BXFMServerPerm.prototype = {
	...
	Request : function(action, postParams, callBack, bShowWaitWin)
	{
		...
		if (bShowWaitWin)
			BX.showWait();
		return BX.ajax.post('/webmin/admin/fileman_server_access.php?lang=' + this.Params.lang + "&fu_action=" + action + "&site=" + this.Params.site + "&" + this.Params.sessid_get, postParams || {},
			function(result)
			{
				if (bShowWaitWin)
					BX.closeWait();
	...
	ShowAskUserDialog: function(Params)
	{
		...
		if (!this.oAskUserDialog)
		{
			...
			setTimeout(function()
			{
				...
				_this.oAskUserDialog.PARTS.CONTENT.appendChild(pAskPialog);
