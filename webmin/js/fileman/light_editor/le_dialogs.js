...
window.LHEDailogs['Anchor'] = function(pObj)
{
	...
	var OnSave = function()
	{
		...
		{
			pObj.pLEditor.InsertHTML('<img id="' + anchorName + '" src="' + pObj.pLEditor.oneGifSrc + '" style="background-image: url(/webmin/images/fileman/light_htmledit/lhe_iconkit.gif); background-position: -260px 0; height: 20px; width: 20px"  __bxtagname="anchor" />');
		}
		pObj.Close();
	};
...
window.LHEDailogs['Video'] = function(pObj)
{
	...
	var OnSave = function()
	{
		...
		if (path.indexOf('http://') != -1 || path.indexOf('.') != -1)
		{
			...
			pVid.style.backgroundImage = (pObj.pPrevPath.value.length > 0) ? 'url(' + pObj.pPrevPath.value + ')' : 'url(/webmin/images/fileman/light_htmledit/video.gif)';
		}
		else
		{
	...
	var PreviewReload = function()
	{
		...
		if (path.indexOf('http://') != -1 || path.indexOf('.') != -1)
		{
			...
			if (ext && (ext == 'wmv' || ext == 'wma')) // WMV
			{
				...
				{
					try{
						new jeroenwijering.Player(document.getElementById("lhe_dialog_preview_wmv"), "/webmin/components/webmin/player/wmvplayer/wmvplayer.xaml", {file: path, width:"320", height:"240", windowless: true});
						pObj.pPreviewDivWMV.style.display = 'block';
						pObj.pPreviewDivFLV.style.display = 'none';
					}catch(e){}
				};
				if (!window.jeroenwijering)
					jsUtils.loadJSFile(['/webmin/components/webmin/player/wmvplayer/silverlight.js', '/webmin/components/webmin/player/wmvplayer/wmvplayer.js'], f1);
				else
					f1();
			}
			else
			{
				try{
				var pEmb = jsUtils.CreateElement('EMBED', {src: "/webmin/components/webmin/player/mediaplayer/player.swf", type:"application/x-shockwave-flash", width:"320", height: "240", allowscriptaccess: "always", allowfullscreen: "true", flashvars:'file=' + path + '&skin=/webmin/components/webmin/player/mediaplayer/skins/bitrix.swf&bufferlength=10'});
