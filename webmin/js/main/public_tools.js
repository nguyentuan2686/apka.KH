...
JCComponentUtils.prototype.ClearCache = function(params)
{
	...
	ShowWaitWindow();
	CHttpRequest.Send('/webmin/admin/clear_component_cache.php?' + params);
};
...
JCComponentUtils.prototype.EnableComponent = function(params)
{
	CHttpRequest.Action = function(result){window.location = window.location.href;};
	ShowWaitWindow();
	CHttpRequest.Send('/webmin/admin/enable_component.php?' + params);
};
...
var jsExtLoader = {
	...
	{
		if (/\/webmin\/js\/main\/public_tools.js$/i.test(script_src)) return; // sorry guys, i cannot execute myself :-)
		if (jsExtLoader.__isScriptLoaded(script_src)) return;

		jsExtLoader.arLoadedScripts.push(script_src);

		if (script_src.substring(0, 8) != '/webmin/')
			script_src = '/webmin/admin/' + script_src;
