...
function JCAdminMenu(sOpenedSections)
{
	...
	this.ToggleDynSection = function(cell, module_id, div_id, level)
	{
		...
		if(div.innerHTML == '')
		{
			...
			this.request.Send('/webmin/admin/get_menu.php?lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+module_id+'&admin_mnu_menu_id='+encodeURIComponent(div_id));
		}
...
function TabControl(name, unique_name, aTabs)
{
	...
	this.ShowWarnings = function(form_name, warnings)
	{
		...
		for(var i in warnings)
		{
			...
			img.src = '/webmin/themes/'+phpVars.ADMIN_THEME_ID+'/images/icon_warn.gif';
	...
	this.SaveSettings =  function()
	{
		...
		var options_url = '/webmin/admin/user_options.php?lang='+phpVars.LANGUAGE_ID+'&sessid='+phpVars.bitrix_sessid;
	...
	this.DisableSettings = function()
	{
		...
		request.Send('/webmin/admin/user_options.php?lang=' + phpVars.LANGUAGE_ID + sParam + '&sessid='+phpVars.bitrix_sessid);
	}
...
var jsAdminChain =
{
	...
	AddItems: function(divId)
	{
		...
		main_chain.innerHTML += '<img src="/webmin/themes/'+phpVars.ADMIN_THEME_ID+'/images/chain_arrow.gif" alt="" border="0" class="arrow">';
...
function JCUserOptions()
{
	...
	this.SendData = function(callback)
	{
		...
		if(sParam != '')
		{
			...
			_this.request.Send('/webmin/admin/user_options.php?'+sParam+'&sessid='+phpVars.bitrix_sessid);
		}
	}
	...
	{
		_this.request.Action = callback;
		_this.request.Send('/webmin/admin/user_options.php?action=delete&c='+sCategory+'&n='+sName+(bCommon == true? '&common=Y':'')+'&sessid='+phpVars.bitrix_sessid);
	}
}
...
function JCPanel()
{
	...
	this.ShowOn = function()
	{
		...
		if(bIE)
		{
			...
			{
				document.body.style.backgroundImage = "url(/webmin/images/1.gif)";
...
function CWizardWindow()
{
	...
	this.Open = function(wizardName, sessid)
	{
		...
		div.innerHTML =
			...
			'</table>'+
			'</div>'+
			'<div class="wizard" style="height:387px;"><iframe class="content" style="background-color: transparent;" allowtransparency="true" scrolling="no" id="wizard_iframe" width="100%" src="/webmin/admin/wizard_install.php?lang='+phpVars.LANGUAGE_ID+'&wizardName='+wizardName+'&sessid='+sessid+'" height="100%" frameborder="0">'+
			''+
			'</iframe></div>';
...
function JCStartMenu()
{
	...
	this.ShowStartMenu = function(button, back_url)
	{
		...
		if(!menuStart || !menuStart.menuItems)
		{
			...
			request.Send('/webmin/admin/get_start_menu.php?lang='+phpVars.LANGUAGE_ID+(back_url? '&back_url_pub='+encodeURIComponent(back_url):'')+'&sessid='+phpVars.bitrix_sessid);
		}
		else
		{
	...
	this.OpenDynMenu = function(menu, module_id, items_id, back_url)
	{
		...
		request.Send('/webmin/admin/get_start_menu.php?mode=dynamic&lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+encodeURIComponent(module_id)+'&admin_mnu_menu_id='+encodeURIComponent(items_id)+(back_url? '&back_url_pub='+encodeURIComponent(back_url):'')+'&sessid='+phpVars.bitrix_sessid);
	}
	...
	this.OpenURL = function(item, arguments, url, back_url)
	{
		...
		{
			request.Action = function(result){}
			request.Send('/webmin/admin/get_start_menu.php?mode=save_recent&url='+encodeURIComponent(url)+'&text='+encodeURIComponent(itemInfo['TEXT'])+'&title='+encodeURIComponent(itemInfo['TITLE'])+'&icon='+itemInfo['ICON']+'&sessid='+phpVars.bitrix_sessid);
		}
		if(back_url)
