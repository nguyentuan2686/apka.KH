...
BX.WindowManager = {
	...
	saveWindowOptions: function(wnd_id, opts)
	{
		if (BX.userOptions)
		{
			for (var i in opts)
			{
				BX.userOptions.save('BX.WindowManager', 'options_' + wnd_id, i, opts[i]);
			}
		}
	},
...
BX.CDialog = function(arParams)
{
	...
	this.PARTS.FOOT = this.DIV.appendChild(BX.create('DIV', {props: {
		...
		html: '<div class="l"><div class="r"><div class="c"><img src="/webmin/js/main/core/images/line.png" height="1" width="90%" border="0" style="position: absolute; top: 0; left: 0;" /><span></span></div></div></div>'
	}));
...
BX.CDialog.prototype.__expand_onresize = function()
{
	...
	var width = (wndSize.innerWidth - parseInt(BX.style(this.PARTS.CONTENT, 'padding-right'))),
		height = (wndSize.innerHeight - this.PARTS.TITLEBAR.offsetHeight - (this.PARTS.FOOT.offsetHeight) + this.dxShadow);
	
	this.PARTS.CONTENT_DATA.style.width = width + 'px';
	this.PARTS.CONTENT_DATA.style.height = height + 'px';
	
	BX.onCustomEvent(this, 'onWindowResize');
	BX.onCustomEvent(this, 'onWindowResizeExt', [{'width': width, 'height': height}]);
}
...
BX.CMenuOpener = function(arParams)
{
	...
	this.bSave = !!this.PARAMS.ID;
	if (!this.bSave) this.PARAMS.ID = 'cc_' + parseInt(10000 * Math.random());
	...
	if (this.EXTRA_BUTTONS['components2_props'])
	{
		...
		if (null == this.defaultAction) 
		{
			...
			this.defaultActionTitle = btn.TITLE;
		}
...
{
	this.DIV.style.display = (val && this.PARAMS.pin) ? 'block' : 'none';
}
...
BX.CMenuOpener.prototype.Transform = function(val)
{
	...
	// {
		// this.OPENERS.MENU.PARAMS.ATTACH_MODE = this.PARAMS.transform ? 'right' : 'bottom';
	// }
...
{
	this.OPENERS[this.OPENERS.length] = new BX.COpener({
		'DIV':  this.OPENER,
		'ATTACH': this.ATTACH,
		'MENU': menu,
		'TYPE': 'click'
...
BX.CMenuOpener.prototype.__restoreZIndex = function(){this.DIV.style.zIndex = this.zIndex;}
BX.CMenuOpener.prototype.__changeZIndex = function(){this.DIV.style.zIndex = BX.WindowManager.GetZIndex();}
	...
	this.LINK = this.WRAPPER.firstChild.firstChild.appendChild(BX.create('A', {
		...
		children: [
			BX.create('SPAN', {
				...
				attrs: (this.item.SRC || this.item.IMAGE) ? {
...
BX.CMenuOpenerItem.prototype.__msover = function() {
	this.parentNode.parentNode.className += ' bx-context-toolbar-button-hover';
}
BX.CMenuOpenerItem.prototype.__msout = function() {
	this.parentNode.parentNode.className = 'bx-context-toolbar-button';
}
...
BX.CMenu.prototype.__adjustMenuToNode = function()
{
	...
	switch (this.PARAMS.ATTACH_MODE)
	{
		case 'bottom': 
			...
			{
				menu_pos.left = scrollSize.scrollWidth - floatWidth - 1;
			}
		break;
		case 'right':
			...
			if (menu_pos.left > scrollSize.scrollWidth - floatWidth)
			{
				menu_pos.left = pos.left - floatWidth;
			}
		break;
	}
