...
class CLightHTMLEditor // LHE
{
	...
	{
		global $USER;
		$basePath = '/webmin/js/fileman/light_editor/';
		...
		$this->videoSettings = is_array($arParams['videoSettings']) ? $arParams['videoSettings'] : array(
				...
				'windowless' => true,
				'bufferLength' => 20,
				'skin' => '/webmin/components/webmin/player/mediaplayer/skins/bitrix.swf',
				'logo' => ''
			);
	...
	function InitScripts()
	{
		...
		function LoadLHE_<?=$this->Id?>() // Load JS and CSS via jsUtils
		{
			...
			if (!window.PopupMenu)
				arScripts.push('<?=$this->GetActualPath('/webmin/js/main/popup_menu.js')?>');
	...
	{
		$langPath = $_SERVER['DOCUMENT_ROOT'].'/webmin/modules/fileman/lang/'.LANGUAGE_ID.'/classes/general/light_editor_js.php';
		if(file_exists($langPath))
			include($langPath);
		else
			$langPath = $_SERVER['DOCUMENT_ROOT'].'/webmin/modules/fileman/lang/en/classes/general/light_editor_js.php';
