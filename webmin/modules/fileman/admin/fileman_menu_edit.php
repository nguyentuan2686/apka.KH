		...
		if($REQUEST_METHOD=="POST" && strlen($save) >0 && is_array($ids) && check_bitrix_sessid())
		{
			...
			if(strlen($apply)<=0)
			{
				if(strlen($back_url)>0)
					...
					LocalRedirect("/webmin/admin/fileman_admin.php?".$addUrl."&site=".$site."&path=".UrlEncode($path));
			}
			else
				LocalRedirect("/webmin/admin/fileman_menu_edit.php?".$addUrl."&site=".$site."&path=".UrlEncode($path)."&name=".$name);
		}
	}
}
...
?>
	...
	$tabControl->Buttons(
		array(
			"disabled" => false,
			"back_url" => ((strlen($back_url)>0 && strpos($back_url, "/webmin/admin/fileman_menu_edit.php")!==0) ? htmlspecialcharsex($back_url) : "/webmin/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["FULL"]))
		)
	);
	?>
