...
require($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/fileman/include.php");
...
function OnSave()
{
	...
	else if(anchor_value.value.length > 0) // New anchor
	{
		...
		pObj.pMainObj.insertHTML('<img id="' + id + '" src="' + one_gif_src + '" style="background-image: url(' + global_iconkit_path + '); background-position: -260px 0; height: 20px; width: 20px" />');
...
function OnLoad()
{
	...
	if(pElement && bxTag) /* Link selected*/
	{
		...
		else if (href.indexOf("://") !== -1 || href.substr(0, 'www.'.length) == 'www.' || href.indexOf("&goto=") !== -1)
		{
			...
			if(href.substr(0, '/webmin/redirect.php'.length) == '/webmin/redirect.php')
			{
				BX("bx_fixstat").checked = true;
				ChangeFixStat();
				var sParams = href.substring('/webmin/redirect.php'.length);
	...
	switch(BX('bx_link_type').value)
	{
		...
		case 't2':
			...
			if(BX("bx_fixstat").checked)
				href = '/webmin/redirect.php?event1=' + escape(BX("event1").value) + '&event2=' + escape(BX("event2").value) + '&event3=' + escape(BX("event3").value) + '&goto=' + escape(href);
			break;
		case 't3':
...
function OnSave()
{
	...
	var arEls = ['src', 'alt', 'title', 'hspace', 'vspace', 'border'], i, l = arEls.length;
	for (i = 0; i < l; i++)
	{
		if (!pObj.pMainObj.pParser.isPhpAttribute(oTag.params[arEls[i]]))
	...
	<table border="0" width="100%" height="100%" style="vertical-align: middle">
		...
		<tr>
			<td align="center" valign="top">
				<img style="vertical-align: middle;" src="/webmin/themes/.default/images/wait.gif" />
...
function _displayTaskbarList(oCont)
{
	...
	if(pObj.pMainObj.allowedTaskbars['BXComponents2Taskbar'])
	{
		...
		if (!window.BXComponents2Taskbar || !ar_BXTaskbarS["BXComponents2Taskbar_" + pObj.pMainObj.name])
		{
			...
			if (!settings.show || !ar_BXTaskbarS["BXComponents2Taskbar_" + pObj.pMainObj.name])
	...
	if (!compareObj(Settings.arTaskbarSettings, window.temp_arTaskbarSettings))
	{
		...
		if(temp_arTaskbarSettings['BXSnippetsTaskbar'].show)
		{
			if (BXSnippetsTaskbar_need_preload)
				arScripts.push("/webmin/admin/htmleditor2/snippets.js");
		}
		...
		{
			if (BXComponents2Taskbar_need_preload)
				arScripts.push("/webmin/admin/htmleditor2/components2.js");
...
function saveSnippet(fileName, templateId, new_group)
{
	...
	_ss.Action = function(result) {setTimeout(function()
	{
		...
		var __arEl =
		{
			...
			thumb:thumb,
			isGroup:false,
			icon:'/webmin/images/fileman/htmledit2/snippet.gif',
...
function OnLoad()
{
	...
	BX.findChild(oDiv, {tag: 'TABLE', className: 'edit-tab-title'}, true).style.display = "block";
	...
	// Insert default HEAD
	newCell.appendChild(BX.create("DIV", {props: {title: '<?=GetMessage("FILEMAN_ED_INSERT_DEF")?>', className: "iconkit_c bx-insert-hbf", onclick: insertDefault_head}}));
	...
	newCell.appendChild(BX.create("DIV", {props: {title: '<?=GetMessage("FILEMAN_ED_INSERT_DEF")?>', className: "iconkit_c bx-insert-hbf", onclick: insertDefault_footer}}));
