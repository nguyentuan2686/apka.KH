...
{
	include($_SERVER["DOCUMENT_ROOT"]."/webmin/admin/fileman_access.php");
	die();
}
...
while($Elem = $db_DirContent->NavNext(true, "f_"))
{
	...
	if($logical == 'Y')
	{
		...
		if($Elem["TYPE"] == "D")
		{
			$showFieldIcon = "<a href=\"javascript:".$sTableID.".GetAdminList('fileman_admin.php?".$addUrl_s."&site=".urlencode($site)."&path=".$fpathUrl."&show_perms_for=".IntVal($show_perms_for)."', GALCallBack);\" title=\"".htmlspecialchars($fpath)."\"><IMG SRC=\"/webmin/images/fileman/types/folder.gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 ALT=\"\"></a>";
		...
		else
		{
			...
			{
				$showFieldIcon = "<IMG SRC=\"/webmin/images/fileman/types/".$curFileType.".gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 ALT=\"\"  title=\"".htmlspecialchars($fpath)."\">";
...
function GALCallBack(url)
{
	...
	{
		tbl_fileman_admin_filter.OnSet('tbl_fileman_admin', '/webmin/admin/fileman_admin.php?lang=' + lang + '&site=' + site + '&path=' + path + '&');
		return false;
	};

	delFilterBut.onclick = function()
	{
		tbl_fileman_admin_filter.OnClear('tbl_fileman_admin', '/webmin/admin/fileman_admin.php?lang=' + lang + '&site=' + site + '&path=' + path + '&');
		return false;
	};
}
</script>
