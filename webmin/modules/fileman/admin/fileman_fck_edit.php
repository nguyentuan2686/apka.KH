...
if(strlen($strWarning)<=0)
{
	...
	if($REQUEST_METHOD=="POST" && strlen($save)>0 && strlen($propeditmore)<=0 && check_bitrix_sessid())
	{
		...
		if(strlen($strWarning)<=0)
		{
			...
			{
				if(strlen($back_url)>0 && strpos($back_url, "/webmin/admin/fileman_fck_edit.php")!==0)
				...
				LocalRedirect("/webmin/admin/fileman_fck_edit.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($path).($full_src=="Y"?"&full_src=Y":""));
...
?>
	...
	<?if(!$bEdit):?>
		...
		<select name="template" class="typeselect" onchange="window.location='/webmin/admin/fileman_file_edit.php?lang=<?echo LANG?>&site=<?=Urlencode($site)?>&path=<?echo UrlEncode($path)?>&new=y&template='+escape(this[this.selectedIndex].value)">
			<?for($i=0; $i<count($arTemplates); $i++):?>
	...
	<?endif?>
		<tr><td colspan="2">
			...
			$oFCKeditor->Config['StylesXmlPath'] = '/webmin/admin/fileman_fck_styles.php?'.$addUrl.'&template='.$template.'&list=y&site='.$site;
			$oFCKeditor->Config['EditorAreaCSS'] = '/webmin/admin/fileman_fck_styles.php?'.$addUrl.'&template='.$template.'&site='.$site;
			$oFCKeditor->Config['LinkBrowserURL'] = '/webmin/admin/fileman_fck_browser.php?'.$addUrl.'&site='.$site;
...
$tabControl->Buttons(array(
	"disabled" => false,
	"back_url" => (strlen($back_url)>0 && strpos($back_url, "/webmin/admin/fileman_fsk_edit.php")!==0) ? htmlspecialchars($back_url) : "/webmin/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"])
));
$tabControl->End();
?>
	</form>
