...
if($ext_html_editor=="always" || ($ext_html_editor=="not_admin" && !$USER->IsAdmin()))
	LocalRedirect("/webmin/admin/fileman_fck_edit.php?".$_SERVER["QUERY_STRING"] + "&" + bitrix_sessid_get());
...
if(strlen($back_url)>0 && strpos($back_url, "/webmin/admin/fileman_file_edit.php")!==0)
	$url = "/".ltrim($back_url, "/");
else
	$url = "/webmin/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"]);
		...
		if(strlen($strWarning) <= 0)
		{
			...
			else
			{
				...
				if(strlen($strWarning)<=0 && strlen($apply)<=0 && strlen($apply2)<=0)
					...
					LocalRedirect("/webmin/admin/fileman_html_edit.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($path)."&back_url=".UrlEncode($back_url)."&fullscreen=".($bFullScreen?"Y":"N")."&tabControl_active_tab=".urlencode($tabControl_active_tab));
			}
...
if($bEdit)
{
	...
	if($USER->CanDoFileOperation('fm_delete_file', $arPath))
	{
		...
		$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("FILEMAN_FILE_DELETE_CONFIRM")."')) window.location='/webmin/admin/fileman_admin.php?ID=".urlencode($id)."&action=delete&".$addUrl."&site=".urlencode($site)."&path=".urlencode($folder_path)."&".bitrix_sessid_get()."';",
...
{
	window.location='/webmin/admin/fileman_html_edit.php?lang=<?= LANG?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($path)?>&new=y&template='+id;
...
?>
	<?if(!$bEdit):?>
		...
		function templateOnChange(_this)
		{
			...
			window.location='/webmin/admin/fileman_html_edit.php?lang=<?= LANG?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($path)?>&new=y&template='+escape(_this[_this.selectedIndex].value)+_name+_title;
		}
		</script>
		<?
		...
		<tr><td style="padding: 0!important;"></td><td style="padding: 0!important;">
			...
			<IMG src="/webmin/themes/.default/images/icon_warn.gif" title="<?=GetMessage("FILEMAN_NAME_ERROR");?>">
	...
	<td colspan="2">
		...
		function OnSaveAs(filename, path, site, title, menu)
		{
			...
			BX("filesrc").pMainObj.SaveContent(true);
			if(apply)
				BX("apply2").value = 'Y';
			BX("filesrc").pMainObj.isSubmited = true;
			BX("filesrc").pMainObj.pForm.submit();
		}
		</script>
		<?
