...
elseif(isset($_GET['action']) && $_GET['action'] == 'redirect' && check_bitrix_sessid())  //Redirect after files uploading
{
	...
	$APPLICATION->SetAdditionalCSS('/webmin/js/fileman/medialib/medialib_admin.css');
	...
	$aContext[] = Array(
		...
		"ICON" => "",
		"LINK" => "/webmin/admin/fileman_medialib_admin.php?lang=".LANGUAGE_ID."&".bitrix_sessid_get(),
	...
	if ($len > 0)
	{
		...
		<form name="ml_mu_form" action="/webmin/admin/fileman_medialib_upload.php?action=postsave" method="POST">
		...
		for($i = 0; $i < $len; $i++)
		{
			...
			<tr>
				...
				<?else:?>
					<img src="/webmin/images/1.gif" class="ml-item-thumb ml-item-no-thumb" />
				<?endif;?>
				</td>
			</tr>
			<tr>
		...
		<input type="button" title="<?= GetMessage('admin_lib_edit_cancel_title')?>" onclick="window.location='/webmin/admin/fileman_medialib_admin.php?lang=<?= LANGUAGE_ID?>';" name="cancel" value="<?= GetMessage('admin_lib_edit_cancel')?>" />
		</form>
		<script>
		...
		function addCollToItem(ItemId, id, pSel)
		{
			...
			var
				...
				pDel = pDiv.appendChild(jsUtils.CreateElement("IMG", {src: '/webmin/images/1.gif', className: 'mu-col-del', id: 'mu_it_' + id, title: '<?= GetMessage('ML_DEL_COL2ITEM')?>'}));
...
elseif(isset($_GET['action']) && $_GET['action'] == 'postsave' && check_bitrix_sessid())
{
	...
	LocalRedirect("/webmin/admin/fileman_medialib_admin.php?lang=".LANGUAGE_ID."&".bitrix_sessid_get());
}
else // ***************************** Show upploader  **************************
{
	?><script type="text/javascript" src="/webmin/image_uploader/iuembed.js"></script><?
	include_once($_SERVER['DOCUMENT_ROOT']."/webmin/image_uploader/version.php");
	include_once($_SERVER['DOCUMENT_ROOT']."/webmin/image_uploader/localization.php");
...
iu.addParam("Action", "/webmin/admin/fileman_medialib_upload.php?action=upload&<?= bitrix_sessid_get()?>&lang=<?= LANGUAGE_ID?>&ml_type=<?= intVal($_GET['type'])?>");
...
function ImageUploaderML_AfterUpload(Html)
{
	try
	{
		...
		return jsUtils.Redirect([], "/webmin/admin/fileman_medialib_upload.php?action=redirect&<?= bitrix_sessid_get()?>&lang=<?= LANGUAGE_ID?>" + sGet);
	}
	catch(e)
	{
		return jsUtils.Redirect([], "/webmin/admin/fileman_medialib_upload.php?<?= bitrix_sessid_get()?>&lang=<?= LANGUAGE_ID?>");
	}
}
