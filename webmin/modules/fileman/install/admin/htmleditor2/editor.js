...
BXHTMLEditor.prototype.OnLoad = function()
{
	...
	this.pEditorFrame = this.cEditor.appendChild(BX.create("IFRAME", {props: {id: "ed_" + name, className: "bx-editor-iframe", src: "javascript:void(0)", frameborder: 0}}));
...
BXHTMLEditor.prototype.SetView = function(sType)
{
	...
	switch(sType)
	{
		case 'code':
			...
			setTimeout(function(){_this.pSourceFrame.focus();},50);
			break;
		case 'split':
	...
	// Dirty hack for Firefox (and Chrome and Safari and Opera :)
	if (sType != 'code' && !BX.browser.IsIE())
	{
		try{
...
{
	//Toolbars
	this.RS_toolbars = window.RS_toolbars !== false;
	SETTINGS[this.name].arToolbarSettings = window.arToolbarSettings;

	//Taskbars
	this.RS_taskbars = window.RS_taskbars !== false;
	SETTINGS[this.name].arTaskbarSettings = window.arTaskbarSettings;

	//Taskbarsets
	SETTINGS[this.name].arTBSetsSettings = window.arTBSetsSettings;

	//Tooltips
	this.showTooltips4Components = window.__show_tooltips;

	// Visual effects
	this.visualEffects = window.__visual_effects;
