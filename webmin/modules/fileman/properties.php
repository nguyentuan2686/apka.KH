...
BX.loadCSS('/webmin/components/webmin/map.google.view/settings/settings.css');
...
function saveYandexKey(domain, input)
{
	...
	ShowWaitWindow();
	CHttpRequest.Post('/webmin/admin/settings.php?lang=<?echo LANGUAGE_ID?>&mid=fileman&save_map_key=Y', data);
}
</script>
<?
...
jsUtils.loadCSSFile('/webmin/components/webmin/map.yandex.view/settings/settings.css');
...
<script>
jsUtils.loadCSSFile("/webmin/components/webmin/player/js/skin_selector.css");
jsUtils.loadJSFile("/webmin/components/webmin/player/js/prop_skin_selector.js", function()
{
...
<?$APPLICATION->IncludeComponent(
	...
	<tr id="bx_video_new_path_row_<?= $id?>" <?if (strlen($path) > 0){ echo 'style="display: none;"'; }?>>
		...
		<img src="/webmin/images/1.gif" class="spacer" />
...
<?
	...
	{
		return "/webmin/components/webmin/player/mediaplayer/skins";
	}
