...
class WizardServices
{
	...
	function ImportIBlockFromXML($xmlFile, $iblockCode, $iblockType, $siteID, $permissions = Array())
	{
		...
		require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/iblock/classes/".strtolower($GLOBALS["DB"]->type)."/cml2.php");
	...
	{
		global $DBType;
		require_once($_SERVER['DOCUMENT_ROOT']."/webmin/modules/main/classes/".strtolower($DBType)."/favorites.php");
	...
	function ReplaceMacrosRecursive($filePath, $arReplace)
	{
		...
		if ($handle = @opendir($filePath))
		{
			while (($file = readdir($handle)) !== false)
			{
				if ($file == "." || $file == ".." || (trim($filePath, "/") == trim($_SERVER["DOCUMENT_ROOT"], "/") && $file == "webmin")  || (trim($filePath, "/") == trim($_SERVER["DOCUMENT_ROOT"], "/") && $file == "upload")) continue;
