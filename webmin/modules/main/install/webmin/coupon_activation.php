...
require_once($_SERVER['DOCUMENT_ROOT']."/webmin/php_interface/dbconn.php");
...
function UpdateSetOption($name, $value)
{
	...
	$fn = $_SERVER['DOCUMENT_ROOT']."/webmin/managed_cache/".strtoupper($DBType)."/e5/".md5("b_option").".php";
...
function UpdateGetHTTPPage($requestDataAdd, &$errorMessage)
{
	...
	$serverIP = UpdateGetOption("update_site", "www.webminsoft.com");
	$serverPort = 80;
	...
	if ($FP)
	{
		...
		if ($bUseProxy)
		{
			$requestString .= "POST http://".$serverIP."/webmin/updates/us_updater_actions.php HTTP/1.0\r\n";
			if (strlen($proxyUserName) > 0)
				$requestString .= "Proxy-Authorization: Basic ".base64_encode($proxyUserName.":".$proxyPassword)."\r\n";
		}
		else
			$requestString .= "POST /webmin/updates/us_updater_actions.php HTTP/1.0\r\n";
...
function UpdateActivateCoupon($coupon, &$errorMessage)
{
	...
	$fp = fopen($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/admin/define.php", 'w');
	...
	{
		$fp = fopen($_SERVER['DOCUMENT_ROOT']."/webmin/license_key.php", "wb");
...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/webmin/themes/.default/sysupdate.css">
	...
	?>
	<form method="POST" action="/webmin/coupon_activation.php">
