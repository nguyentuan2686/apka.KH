			...
			if ($sPerm1 >= 'W')
			{
				...
				if($USER->CanDoOperation('edit_php'))
				{
					$arIcons[] = array(
						...
						"ICON" => "bx-context-toolbar-edit-icon",
						"TITLE"=>GetMessage("main_comp_include_edit_php"),
						"ALT" => GetMessage("MAIN_INCLUDE_AREA_EDIT_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR"),
					);
				}
			}
			...
			if ($sFilePath != $sFilePathTMP && $sPerm2 >= 'W')
			{
				...
				if($USER->CanDoOperation('edit_php'))
				{
					$arIcons[] = array(
						...
						"ICON" => "bx-context-toolbar-create-icon",
						"TITLE"	=> GetMessage("main_comp_include_add_php"),
						"ALT" => GetMessage("MAIN_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR"),
					);
				}
			}
		}
		elseif ($sPerm2 >= 'W')
		{
			...
			if($USER->CanDoOperation('edit_php'))
			{
				$arIcons[] = array(
					...
					"ICON" => "bx-context-toolbar-create-icon",
					"TITLE" => GetMessage("main_comp_include_add1_php"),
					"ALT" => GetMessage("MAIN_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR"),
				);
			}
		}
