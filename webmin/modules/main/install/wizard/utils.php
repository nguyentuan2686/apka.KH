...
class BXInstallServices
{
	function CreateWizardIndex($wizardName, &$errorMessage)
	{
		$indexContent = '<'.'?'.
			...
			'require_once('.'$'.'_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/wizard.php");'.
			'$'.'wizard = new CWizard("'.$wizardName.'");'.
			'$'.'wizard->Install();'.
			'require($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/include/epilog_after.php");'.
			'?'.'>';
	...
	function LoadWizardData($wizard)
	{
		...
		elseif (count($ar) == 2)
			$path = $_SERVER["DOCUMENT_ROOT"]."/webmin/wizards/".$ar[0]."/".$ar[1];
		else
			return false;
		...
		if (strlen($arWizardDescription["IMAGE"]) > 0)
		{
			if (count($ar) > 2)
			{
				BXInstallServices::CopyDirFiles(
					...
					$_SERVER["DOCUMENT_ROOT"]."/webmin/tmp/".$ar[1]."/".$ar[2]."/".$arWizardDescription["IMAGE"],
					true,
					(defined("BX_DIR_PERMISSIONS") ? (BX_DIR_PERMISSIONS) : 0755),
					(defined("BX_FILE_PERMISSIONS") ? (BX_FILE_PERMISSIONS) : 0644)
				);
				$arWizardDescription["IMAGE"] = "/webmin/tmp/".$ar[1]."/".$ar[2]."/".$arWizardDescription["IMAGE"];
			}
			else
			{
				$arWizardDescription["IMAGE"] = "/webmin/wizards/".$ar[0]."/".$ar[1]."/".$arWizardDescription["IMAGE"];
			}
		}
	...
	function GetWizardsList($moduleName = "")
	{
		...
		{
			$path = $_SERVER["DOCUMENT_ROOT"]."/webmin/wizards";
			if ($h1 = opendir($path))
			{
		...
		$path = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules";
		if ($h1 = opendir($path))
		{
			while (($f1 = readdir($h1)) !== false)
			{
		...
		if(LANGUAGE_ID != 'ru' && $arWizardsListD[0] == 'webmin:demo')
		{
	...
	function GetDBTypes()
	{
		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/mysql/database.php"))
			$arTypes["mysql"] = function_exists("mysql_connect");

		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/oracle/database.php"))
			$arTypes["oracle"] = function_exists("OCILogon");

		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/mssql/database.php"))
			$arTypes["mssql"] = function_exists("odbc_connect");

		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/mssql/database_ms.php"))
	...
	function Add2Log($sText, $sErrorCode = "")
	{
		...
		$READ_PSIZE = 8000;
		$LOG_FILE = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/install.log";
		$LOG_FILE_TMP = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/install_tmp.log";
	...
	function GetDemoWizard()
	{
		...
		foreach ($arWizards as $arWizard)
		{
			...
			if ($wizardID == "webmin:demo")
			{
				$defaultWizard = "webmin:demo";
				continue;
			}
		...
		while (($file = readdir($handle)) !== false)
		{
			...
			else
			{
				if (
					...
					strpos($filePath, "/webmin/modules/main/lang/".LANGUAGE_ID."/install.php") !== false
				)
					continue;
