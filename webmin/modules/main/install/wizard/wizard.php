...
require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/wizard.php"); //Wizard API
require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/version.php"); //Sitemanager version
require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/install/wizard/template.php"); //Wizard template
require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/install/wizard/utils.php"); //Wizard utils
...
//Lang files
include($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/lang/en/install.php");
if (LANGUAGE_ID != "en" && file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/lang/".LANGUAGE_ID."/install.php"))
	include($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/lang/".LANGUAGE_ID."/install.php");

//Magic quotes
include_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/tools.php");
UnQuoteAll();
...
class DBTypeStep extends CWizardStep
{
	function InitStep()
	{
		...
		if(file_exists($_SERVER['DOCUMENT_ROOT'].'/webmin/license_key.php'))
		{
			$LICENSE_KEY = '';
		...
		if(defined("TRIAL_VERSION") || defined("TRIAL_RENT_VERSION"))
		{
			...
			if((defined("TRIAL_RENT_VERSION") || (defined("TRIAL_VERSION") && $lic_key_variant == "Y")) && strlen($licenseKey) <= 0)
			{
				...
				if(!$bError)
				{
					...
					$handle = @opendir($_SERVER["DOCUMENT_ROOT"]."/webmin/modules");
					if ($handle)
					{
						while (false !== ($dir = readdir($handle)))
						{
							if (is_dir($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$dir)
								&& $dir!="." && $dir!="..")
							{
								$module_dir = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$dir;
...
*/
	...
	function CheckFileAccess()
	{
		$this->arCheckFiles = Array(
			...
			Array("PATH" => $_SERVER["DOCUMENT_ROOT"]."/webmin", "DESC" => InstallGetMessage("SC_DISK_BITRIX"), "RESULT" => "", "ACCESS" => true),
			Array("PATH" => $_SERVER["DOCUMENT_ROOT"]."/index.php", "DESC" => InstallGetMessage("SC_FILE"), "RESULT" => "", "ACCESS" => true),
			Array("PATH" => $_SERVER["DOCUMENT_ROOT"]."/webmin/modules", "DESC" => InstallGetMessage("SC_CATALOG"), "RESULT" => "", "ACCESS" => true),
		);
	...
	{
		$htaccessTest = $_SERVER["DOCUMENT_ROOT"]."/webmin/httest";
		...
		$arFiles = Array(
			Array(
				"PATH" => $_SERVER["DOCUMENT_ROOT"]."/webmin/httest/.htaccess",
				"CONTENT" => 'ErrorDocument 404 /webmin/httest/404.php

<IfModule mod_rewrite.c>
RewriteEngine On
RewriteCond %{REQUEST_FILENAME} !-f
RewriteRule ^.+\.php$ /webmin/httest/404.php
</IfModule>',
			),
			Array(
				"PATH" => $_SERVER["DOCUMENT_ROOT"]."/webmin/httest/404.php",
				"CONTENT" => "<"."?\n".
			...
			<tr>
				...
				<script type="text/javascript">
					...
					if (req)
					{
						...
						req.open("GET", "/webmin/httest/test_404.php?random=" + Math.random(), true);
						req.send("");
					}
					else
...
class CreateDBStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		BXInstallServices::CheckDirPath($_SERVER["DOCUMENT_ROOT"]."/webmin/admin/", octdec($this->folderPermission));
		if(!is_readable($_SERVER["DOCUMENT_ROOT"]."/webmin/admin/") || !is_writeable($_SERVER["DOCUMENT_ROOT"]."/webmin/admin/"))
		{
			$this->SetError("Access denied: /webmin/admin/");
			return;
		}

		// define.php
		$definePath = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/admin/define.php";
		if(file_exists($definePath))
		{
			if(!is_readable($definePath) || !is_writeable($definePath))
			{
				$this->SetError("Access denied: /webmin/modules/main/admin/define.php");
				return;
			}
		}
		else
		...
		if($this->dbType == "mssql" && $this->DBSQLServerType=="NATIVE")
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$this->dbType."/database_ms.php");
		else
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$this->dbType."/database.php");
		require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$this->dbType."/main.php");
		require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/tools.php");
		IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/main.php");
		...
		BXInstallServices::DeleteDirRec($_SERVER["DOCUMENT_ROOT"]."/webmin/managed_cache");
		BXInstallServices::DeleteDirRec($_SERVER["DOCUMENT_ROOT"]."/webmin/stack_cache");
	}
	...
	function CreateDBConn()
	{
		...
		$fileContent = "<"."?\n".
			...
			"@set_time_limit(60);\n".
			"\n".
			(file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/intranet") ? "@ini_set(\"memory_limit\", \"1024M\");\n" : "").
			$iconv.
	...
	function CreateLicenseFile()
	{
		...
		$filePath = $_SERVER["DOCUMENT_ROOT"]."/webmin/license_key.php";
...
class CreateModulesStep extends CWizardStep
{
	...
	function InstallSingleStep($code)
	{
		...
		if ($code == "remove_help" && $removeHelp)
			BXInstallServices::DeleteDirRec($_SERVER["DOCUMENT_ROOT"]."/webmin/help");
		...
		{
			BXInstallServices::DeleteDirRec($_SERVER["DOCUMENT_ROOT"]."/webmin/httest");
	...
	function &GetModuleObject($moduleID)
	{
		if(!class_exists('CModule'))
		{
			...
			require_once($_SERVER['DOCUMENT_ROOT']."/webmin/modules/main/include.php");
		}

		$installFile = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$moduleID."/install/index.php";
	...
	function InstallModule($moduleID, $currentStepStage)
	{
		if ($moduleID == "main")
		{
			...
			require_once($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/php_interface/dbconn.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/module.php");
			if($DBType=="mssql" && $DBSQLServerType=="NATIVE")
				require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/mssql/database_ms.php");
			else
				require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$DBType."/database.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$DBType."/main.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/cache.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$DBType."/usertype.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$DBType."/user.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$DBType."/option.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$DBType."/event.php");
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/".$DBType."/agent.php");
		}
		else
		{
			global $DB, $DBType, $DBHost, $DBLogin, $DBPassword, $DBName, $DBDebug, $DBDebugToFile, $APPLICATION, $USER, $DBSQLServerType;
		...
		elseif ($currentStepStage == "utf8")
		{
			if (!$this->IsModuleEncode($moduleID))
			{
				...
				BXInstallServices::EncodeDir($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$moduleID, INSTALL_CHARSET);
	...
	function EncodeDemoWizard()
	{
		...
		$path = $_SERVER["DOCUMENT_ROOT"]."/webmin/wizards/webmin";
		if($dir = opendir($path))
		{
	...
	function ShowStep()
	{
		...
		<div id="wait" align=center>
			...
			<table width=200 cellspacing=0 cellpadding=0 border=0 style="border:1px solid #EFCB69" bgcolor="#FFF7D7">
				<tr>
					<td height=50 width="50" valign="middle" align=center><img src="/webmin/images/install/wait.gif"></td>
...
class CreateAdminStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		if(!file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/intranet") && !defined("FIRST_EDITION"))
		...
		if (count($arWizardsList) <= 0)
		{
			...
			if ($wizardName)
			{
				if (BXInstallServices::CreateWizardIndex($wizardName, $errorMessageTmp))
				{
					...
					if (defined("BX_UTF"))
						BXInstallServices::EncodeFile($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/lang/".LANGUAGE_ID."/install.php", INSTALL_CHARSET);
...
class SelectWizardStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		if (count($ar) > 2)
		{
			$path = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$ar[0]."/install/wizards/".$ar[1]."/".$ar[2];
			...
			BXInstallServices::CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$ar[0]."/install/wizards/".$ar[1]."/".$ar[2],
				$_SERVER["DOCUMENT_ROOT"]."/webmin/wizards/".$ar[1]."/".$ar[2],
				true,
		...
		if (!file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/wizards/".$ar[0]."/".$ar[1])
			|| !is_dir($_SERVER["DOCUMENT_ROOT"]."/webmin/wizards/".$ar[0]."/".$ar[1]))
		{
			$this->SetError(InstallGetMessage("INS_WIZARD_NOT_FOUND"));
			return;
		}
	...
	function ShowStep()
	{
		...
		$this->content .= '<a class="solution-item" href="javascript:void(0);" onclick="SelectSolution(this, \'@\'); return false;" ondblclick="document.forms[\''.htmlspecialchars($wizard->GetFormName()).'\'].submit();">
			...
			<div class="solution-inner-item">
				...
				<img alt="" src="/webmin/images/install/marketplace.gif" class="solution-image" />
...
class LoadModuleStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		if (!file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$selectedModule))
		{
			require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/update_client_partner.php");
...
class FinishStep extends CWizardStep
{
	...
	function CreateNewIndex()
	{
		...
		$success = @fwrite($handler,
			...
			'<br /><a href="/webmin/admin/">Control panel</a>'."\n".
	...
	function ShowStep()
	{
		...
		<b><a href="/webmin/admin/sysupdate.php?lang='.LANGUAGE_ID.'">'.InstallGetMessage("GO_TO_REGISTER").'</a></b><br>'.InstallGetMessage("GO_TO_REGISTER_DESCR").'<br><br>

		<table width="100%" cellpadding="2" cellspacing="0" border="0">
			<tr>
				<td><a href="/webmin/admin/index.php?lang='.LANGUAGE_ID.'"><img src="/webmin/images/install/admin.gif" width="22" height="22" border="0" title="'.InstallGetMessage("GO_TO_CONTROL").'"></a></td>
				<td width="50%">&nbsp;<a href="/webmin/admin/index.php?lang='.LANGUAGE_ID.'">'.InstallGetMessage("GO_TO_CONTROL").'</a></td>
				<td><a href="/"><img border="0" src="/webmin/images/install/public.gif" width="22" height="22" title="'.InstallGetMessage("GO_TO_VIEW").'"></a></td>
