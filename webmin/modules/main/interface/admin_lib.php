...
define("ADMIN_THEMES_PATH", "/webmin/themes");
...
<script type=\"text/javascript\" src=\"/webmin/js/main/utils.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/webmin/js/main/utils.js')."\"></script>
<script type=\"text/javascript\" src=\"/webmin/js/main/admin_tools.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/webmin/js/main/admin_tools.js')."\"></script>
<script type=\"text/javascript\" src=\"/webmin/js/main/popup_menu.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/webmin/js/main/popup_menu.js')."\"></script>
";
		...
		if($USER->IsAuthorized() && !isset($_COOKIE[COption::GetOptionString("main", "cookie_name", "BITRIX_SM").'_SOUND_LOGIN_PLAYED']))
		{
			...
			if($aUserOptGlobal["sound"] == 'Y')
			{
				if($aUserOptGlobal["sound_login"] == '')
					$aUserOptGlobal["sound_login"] = "/webmin/sounds/main/bitrix_tune.mp3";

				ob_start();
				$APPLICATION->IncludeComponent("webmin:player",	"",
					Array(
						"PLAYER_TYPE" => "flv",
				...
				$res = '
<script type="text/javascript" src="/webmin/components/webmin/player/mediaplayer/flvscript.js'.'?v='.filemtime($_SERVER['DOCUMENT_ROOT'].'/webmin/components/webmin/player/mediaplayer/flvscript.js').'"></script>
	...
	function _SetActiveItems(&$aMenu, $aSections=array())
	{
		...
		if($aMenu["url"] <> "" || $bMoreUrl)
		{
			...
			for($j = 0; $j < $n; $j++)
			{
				...
				if(strpos($all_links[$j], "/webmin/admin/") !== 0)
					$tested_link = "/webmin/admin/".$all_links[$j];
				else
...
/*
class CAdminPopup extends CAdminPopupOld
{
	function CAdminPopup($name, $id, $items=false, $params=false)
	{
		//SEPARATOR, ID, ONCLICK, ICONCLASS, TEXT, DEFAULT=>true|false, DISABLED=>true|false
...
class CAdminResult extends CDBResult
{
	...
	function GetNavPrint($title, $show_allways=true, $StyleText="", $template_path=false, $arDeleteParam=false)
	{
		if($template_path === false)
			$template_path = $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/interface/navigation.php";
...
class CAdminMainChain extends CAdminChain
{
	...
	function Init()
	{
		...
		parent::AddItem(array("TEXT"=>'<img src="'.ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/images/home.gif" alt="" border="0" class="home">'.GetMessage("admin_lib_navchain_first"), "LINK"=>"/webmin/admin/index.php?lang=".LANGUAGE_ID));
...
class CAdminCalendar
{
	function ShowScript()
	{
		...
		return
"<script type=\"text/javascript\" src=\"/webmin/js/main/calendar.js".'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/webmin/js/main/calendar.js')."\"></script>".
"<script>
var mess = {
	...
	function ShowUserFields($PROPERTY_ID, $ID, $bVarsFromForm)
	{
		...
		if($USER_FIELD_MANAGER->GetRights($PROPERTY_ID) >= "W")
		{
			...
			?>
				<tr colspan="2">
					<td align="left">
						<a href="/webmin/admin/userfield_edit.php?lang=<?echo LANGUAGE_ID?>&amp;ENTITY_ID=<?echo urlencode($PROPERTY_ID)?>&amp;back_url=<?echo urlencode($APPLICATION->GetCurPageParam()."&tabControl_active_tab=user_fields_tab")?>"><?echo $this->GetCustomLabelHTML()?></a>
					</td>
				</tr>
			<?
...
function ShowJSHint($text, $arParams=false)
{
	...
	$res .= '<img src="/webmin/images/1.gif" onload="if (window.hintonload_'.$tmp_id.'){hintonload_'.$tmp_id.'(this);}">';
