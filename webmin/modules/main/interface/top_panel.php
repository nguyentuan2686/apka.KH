...
if($USER->IsAuthorized())
{
	...
	if($USER->CanDoOperation('install_updates'))
	{
		...
		$arPanelButtons[] = array(
			...
			"LINK"=>"/webmin/admin/sysupdate.php?lang=".LANGUAGE_ID,
...
if(!defined("BX_AUTH_FORM") && $USER->IsAuthorized() && CModule::IncludeModule("bizproc"))
{
	...
	if($dbResultList->Fetch())
	{
		...
		$arPanelButtons[] = array(
			...
			"LINK"=> "/webmin/admin/bizproc_task_list.php?lang=".LANGUAGE_ID,
...
<div id="bx-panel" class="bx-panel-admin-mode">
    ...
    <div id="bx-panel-top">
		...
		if($USER->IsAuthorized())
		{
			...
			if ($bCanProfile)
				$userInfo .= '<a href="/webmin/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$USER->GetID().'" id="bx-panel-user">'.htmlspecialchars($USER->GetFullName()).' ('.htmlspecialchars($USER->GetLogin()).')</a><span class="bx-panel-userinfo-separator"></span>';
			else
	...
	<div id="bx-panel-admin-toolbar">
		...
		foreach($arPanelButtons as $item)
		{
			...
			{				
				...
				{
					?><a class="bx-panel-admin-button<?=($item["SELECTED"] == true ? " bx-panel-admin-button-selected" : "" )?>" href="<?=$item["LINK"]?>" <?=(!empty($item["LINK_PARAM"]) ? $item["LINK_PARAM"] : "")?> hidefocus="true" title="<?=$item["TITLE"]?>"><?if (!empty($item["ICON"])):?><span class="bx-panel-admin-button-icon <?=$item["ICON"]?>"></span><?endif?><span class="bx-panel-admin-button-text"><?=$item["TEXT"]?></span></a><?
				}
			}
		}
		?>
