...
if(array_key_exists("show_lang_files", $_GET) || array_key_exists("SHOW_LANG_FILES", $_SESSION))
	include_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/interface/lang_files.php");
...
if($bShowStat && !$USER->IsAuthorized())
{
	...
	$GLOBALS["APPLICATION"]->AddHeadString('<script type="text/javascript" src="/webmin/js/main/public_tools.js"></script>');
	$GLOBALS["APPLICATION"]->AddHeadString('<link rel="stylesheet" type="text/css" href="/webmin/themes/.default/pubstyles.css" />');
}
...
if(!IsModuleInstalled("compression") && !defined('PUBLIC_AJAX_MODE'))
{
	if($bShowTime || $bShowStat)
		include_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/interface/debug_info.php");
}
