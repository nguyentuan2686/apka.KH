...
if ($_REQUEST['unique_id'] && $_REQUEST['unique_id'] == checker_get_unique_id())
{
	...
	elseif ($_GET['upload_test']) 
	{
		...
		$image = $_SERVER['DOCUMENT_ROOT'].'/webmin/site_checker.gif';

		if (move_uploaded_file($tmp_name, $image))
		{
			$BinaryData0 = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/webmin/images/1.gif');
...
require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/prolog.php");
...
//////////   PARAMS   //////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////
define(SUPPORT_PAGE, LANGUAGE_ID == 'ru' ? 'http://www.1c-webmin.ru/support/' : 'http://www.webminsoft.com/support/');
	...
	<?
		...
		else
		{
			foreach($arDocRoot as $root)
			{
				if ($root)
				{
					if (!is_readable($root.'/webmin'))
					{
						$ms_req = GetMessage('SC_ROOT_ACCESS');
						$ms_cur = GetMessage('SC_NO_ACCESS').' '.$root.'/webmin';
						$ms_ok = false;
						break;
					}
				}
...
if (false && $send_ticket=="Y")
{
	...
	if (strlen($strTicketError)<=0)
	{
		...
		{
			LocalRedirect("/webmin/admin/site_checker.php?lang=".LANGUAGE_ID."&ticket_sent=Y&tabControl_active_tab=edit5&ticket_email=".$ticket_email);
		}
		else
		{
	...
	LocalRedirect("/webmin/admin/site_checker.php?lang=".LANGUAGE_ID."&tabControl_active_tab=edit5&strTicketError=".urlencode($strTicketError)."&ticket_sent=N&ticket_text=".urlencode($ticket_text)."&ticket_email=".urlencode($ticket_email));
}
?>
<tr><td colspan="2"><?
...
//			array('check_basename' => GetMessage('SC_T_BASENAME')), # http://bugs.php.net/bug.php?id=33898
	...
	{
		$BinaryData = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/webmin/images/1.gif');
		if (!$BinaryData)
			return $this->Result(false,GetMessage('SC_CHECK_FILES'));
