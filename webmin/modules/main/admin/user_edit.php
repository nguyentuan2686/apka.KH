...
while ($opt_res = $db_opt_res->Fetch())
{
	...
	if (file_exists($DOCUMENT_ROOT.BX_ROOT."/modules/".$mdir) && is_dir($DOCUMENT_ROOT.BX_ROOT."/modules/".$mdir))
	{
		...
		{
			include(GetLangFileName($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$mdir."/lang/", "/options_user_settings.php"));
...
if($USER->CanDoOperation('view_all_users'))
{
	$aMenu[] = array(
		...
		"LINK"	=> "/webmin/admin/user_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
		"ICON"	=> "btn_list",
...
if($USER->CanDoOperation('edit_all_users') || $USER->CanDoOperation('edit_subordinate_users'))
{
	if ($ID>0 && $COPY_ID<=0)
	{
		...
		$aMenu[] = array(
			...
			"LINK"	=> "/webmin/admin/user_edit.php?lang=".LANGUAGE_ID,
			"ICON"	=> "btn_new",
			"TITLE"	=> GetMessage("MAIN_NEW_RECORD_TITLE"),
		);
		$aMenu[] = array(
			"TEXT"	=> GetMessage("MAIN_COPY_RECORD"),
			"LINK"	=> "/webmin/admin/user_edit.php?lang=".LANGUAGE_ID.htmlspecialchars("&COPY_ID=").$ID,
			"ICON"	=> "btn_copy",
		...
		if ($ID!=1)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/webmin/admin/user_admin.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
				"ICON"	=> "btn_delete",
		...
		<td colspan="2" align="center"><table border="0" cellpadding="0" cellspacing="0" align="center">
			...
			while ($arGroups = $dbGroups->Fetch())
			{
				...
				<tr>
					...
					<td width="0%" nowrap><label for="GROUP_ID_ACT_ID_<?= $ind ?>"><?=$arGroups["NAME"]?> [<a href="/webmin/admin/group_edit.php?ID=<?=$arGroups["ID"]?>&lang=<?=LANGUAGE_ID?>" title="<?=GetMessage("MAIN_VIEW_GROUP")?>"><?echo intval($arGroups["ID"])?></a>]</label></td>
