	...
	$context = new CAdminContextMenu(
		array(
			array(
				...
				"LINK"	=> "/webmin/admin/type_admin.php?lang=".LANGUAGE_ID,
	...
	$context = new CAdminContextMenu(
		array(
			...
			array(
				...
				"LINK"	=> "/webmin/admin/type_edit.php?lang=".LANGUAGE_ID,
				"TITLE"	=> GetMessage("MAIN_NEW_RECORD_TITLE"),
				"ICON"	=> "btn_new"
			),
			array(
				"TEXT"	=> GetMessage("MAIN_DELETE_RECORD"),
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/webmin/admin/type_admin.php?ID=".urlencode($arParams["EVENT_NAME"])."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."&action=delete';",
			...
			array(
				...
				"LINK"	=> "/webmin/admin/message_admin.php?find_event_type=".urlencode($arParams["EVENT_NAME"])."&amp;lang=".LANGUAGE_ID,
				"TITLE"	=> GetMessage("TEMPLATE_LIST_TITLE"),
				"ICON"	=> "btn_list"
			),
			array(
				"TEXT"	=> GetMessage("MAIN_NEW_TEMPLATE"),
				"LINK"	=> "/webmin/admin/message_edit.php?EVENT_NAME=".urlencode($arParams["EVENT_NAME"])."&type=this&lang=".LANGUAGE_ID,
...
?><tr valign="top" valign="top">
	<td width="40%">[<a href="/webmin/admin/message_edit.php?ID=<?=$v["ID"]?>"><?=$v["ID"]?></a>]<?=(strLen(trim($v["SUBJECT"])) > 0 ? " " : "").htmlspecialcharsEx($v["SUBJECT"])?>:</td>
	<td width="60%"><?
	$arLID = Array();
