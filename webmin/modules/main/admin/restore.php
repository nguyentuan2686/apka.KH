...
if(isset($_SERVER["BX_PERSONAL_ROOT"]) && $_SERVER["BX_PERSONAL_ROOT"] <> "")
	...
	define("BX_PERSONAL_ROOT", "/webmin");
...
$dbconn = $_SERVER['DOCUMENT_ROOT']."/webmin/php_interface/dbconn.php";
...
if ($Step == 2 && !$bSelectDumpStep)
{
	if ($_REQUEST['source']=='download')
	{
		...
		if (!$url)
			$url = $_REQUEST['arc_down_site'].'/webmin/backup/'.$_REQUEST['arc_down_name'];
		...
		if(count($arDName))
		{
			$ar = array(
				...
				'TEXT' => 
					...
					(count($arDName)>1
					?
					getMsg("DB_SELECT").' <select class="selectitem" name="dump_name"><option>'.(implode('</option><option>',$arDName)).'</option></select>'
					:
				...
				'BOTTOM' => 
				'<input type="button" value="'.getMsg('BUT_TEXT_BACK').'" onClick="document.location=\'/restore.php?Step=1&lang='.LANG.'\'"> '.
				'<input type="button" class="selectitem" id="start_button" value="'.getMsg("BASE_RESTORE", LANG).'" onClick="if(document.restore.dump_name.value==\'\') alert(\''.getMsg("ERR_NO_DUMP", LANG).'\'); else return reloadPage(3, \''. LANG.'\');">'
			);
			html($ar);
			?>
			<script>
...
elseif($Step == 3)
{
	...
	if($r && !$oDB->is_end())
	{
		...
		$oDB->close();
		$arc_name = $_REQUEST["arc_name"];
		SetCurrentProgress($d_pos,filesize($_SERVER['DOCUMENT_ROOT'].'/webmin/backup/'.$_REQUEST['dump_name']));
		$text = 
		$status . '
		<input type="hidden" name="arc_name" value="'.$arc_name.'">
		<input type="hidden" name="dump_name" id="dump_name_id" value="'. $_REQUEST["dump_name"].'">
...
elseif($Step == 4)
{
	...
	{
		@unlink($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/backup/".$_REQUEST["dump_name"]);
...
class CTarRestore extends CTar
{
	function readHeader($Long = false)
	{
		...
		if (is_array($header))
		{
			...
			if ($f == $dr.'restore.php')
				return true;
			elseif ($f == $dr.'.htaccess' && file_exists($_SERVER['DOCUMENT_ROOT'].'/.htaccess'))
