...
require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/tar_gz.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/fileman/fileman.php");
		...
		switch ($NS['step'])
		{
			...
			case 3:
				...
				if (is_object($DirScan))
					$status_msg.= '<br>'.GetMessage('DUMP_CUR_PATH').' <b>'.substr($DirScan->nextPath,strlen($_SERVER['DOCUMENT_ROOT'])).'</b>';
...
/*			GetMessage("MAIN_DUMP_ARC_SIZE")." <b>".HumanSize(filesize($NS["arc_name"]))."</b>" */
			'<br>'.GetMessage('TIME_SPENT').' <b>'.HumanTime($arRes["time"]).'</b>';
		}
...
while($Elem = $rsDirContent->NavNext(true, "f_"))
{
	...
	$showFieldIcon = "<IMG SRC=\"/webmin/images/fileman/types/".$curFileType.".gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 ALT=\"\">";
...
function restoreDump(val)
{
	...
	<tr>
		...
		<td>
			...
			<?
				...
				{
					list($k,$v) = each($val);
					echo '<option value="'.htmlspecialchars($k).'"'.($path == $_SERVER['DOCUMENT_ROOT'] ? ' selected' : '').'>'.htmlspecialchars($v).'</option>';
				}
			?>
			</select>
		</td>
	...
	<tr>
		<td>&nbsp;</td>
		<td><label><input type="checkbox" <?=$strDisableNotMysql?> name="dump_index" id="dump_index" value="Y"<?=COption::GetOptionString("main", "dump_base_index")!="Y" ? "checked" : "" ?>> <? echo GetMessage("MAIN_DUMP_BASE_SINDEX")." (".getTableSize("b_search_content")." ".GetMessage("MAIN_DUMP_BASE_SIZE").")" ?></label>
		</td>
	</tr>
	<tr class="heading">
...
function getArcName()
{
	...
	if(!file_exists($_SERVER['DOCUMENT_ROOT'].BX_PERSONAL_ROOT."/backup/index.php"))
	{
		...
		fwrite($f,"<head><meta http-equiv=\"REFRESH\" content=\"0;URL=/webmin/admin/index.php\"></head>");
		fclose($f);
	}

	if(is_dir($_SERVER['DOCUMENT_ROOT'].BX_PERSONAL_ROOT."/backup") && (is_writable($_SERVER['DOCUMENT_ROOT'].BX_PERSONAL_ROOT."/backup")))
	{
		$arc_name = $_SERVER['DOCUMENT_ROOT'].BX_PERSONAL_ROOT."/backup/".date("Y-m-d.H-i-s.");
		if ($_REQUEST['dump_site_id'])
...
function ignorePath($path)
{
	...
	$path_kernel = array(
		"/webmin/admin",
		"/webmin/activities/webmin",
		"/webmin/components/webmin",
		"/webmin/images",
		"/webmin/js",
		"/webmin/modules",
		"/webmin/sounds",
		"/webmin/themes/.default",
		"/webmin/tools",
		"/webmin/wizards/webmin"
	);
	...
	if ($dump_public xor $dump_kernel)
	{
		foreach($path_kernel as $value)
		{
			...
			elseif ($dump_kernel && $path == substr($_SERVER['DOCUMENT_ROOT'].$value, 0, strlen($path)))
