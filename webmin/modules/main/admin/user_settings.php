...
else
{
	...
	if($aUserOpt["sound_login"] == "") $aUserOpt["sound_login"] = "/webmin/sounds/main/bitrix_tune.mp3";
}
...
if($theme["PREVIEW"] <> "")
{
	...
	if(file_exists($_SERVER["DOCUMENT_ROOT"].$previewName))
	{
		...
		if($aSize !== false)
		{
			...
			echo '<a title="'.GetMessage("user_sett_screen").'" href="'.htmlspecialchars($previewName).'" onclick="ImgShw(\''.htmlspecialchars(CUtil::JSEscape($previewName)).'\', '.$aSize[0].', '.$aSize[1].', \'\'); return false;"><img src="/webmin/images/main/preview.gif" width="16" height="16" alt="'.GetMessage("user_sett_screen").'" border="0"></a>';
		}
	}
}
?>
...
CAdminFileDialog::ShowScript(
	...
	(
		...
		"arPath" => Array('PATH' => '/webmin/sounds/main/'),
...
<?if(file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/security/install/index.php")):?>
...
<?endif;?>
<?if(file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/perfmon/install/index.php")):?>
