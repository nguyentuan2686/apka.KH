...
function Calendar($sFieldName, $sFormName="skform", $sFromName="", $sToName="")
{
	...
	if($bCalendarCode <> "Y")
	{
		...
		$func =
			...
			"	}\n".
			"	window.open('/webmin/tools/calendar.php?lang=".LANGUAGE_ID.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"&admin_section=N")."&'+params+'&date='+escape(dateVal)+'&initdate='+escape(dateVal),'','scrollbars=no,resizable=yes,width='+width+',height='+height+',left='+left+',top='+top);\n".
			"}\n".
			"//-->\n".
			"</script>\n";
	}
...
function convert_to_href($url, $link_class="", $event1="", $event2="", $event3="", $script="")
{
	...
	{
		$script = strlen($script)>0 ? $script : "/webmin/redirect.php";
		$goto = $script.
...
function GetLangFileName($before, $after, $lang=false)
{
	...
	if(strpos($before, "/webmin/modules/")===false)
		return $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/lang/en/tools.php";
	...
	IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$module.$path.$after, $lang);
	return $_SERVER["DOCUMENT_ROOT"]."/webmin/modules/".$module."/lang/".$lang.$path.$after;
}
...
function FindUserID($tag_name, $tag_value, $user_name="", $form_name = "form1", $tag_size = "3", $tag_maxlength="", $button_value = "...", $tag_class="typeinput", $button_class="tablebodybutton", $search_page="/webmin/admin/user_search.php")
{
	global $APPLICATION;
...
function Ch".$tag_name_x."()
{
	...
	if (tv".$tag_name_x."!=document.".$form_name."['".$tag_name."'].value)
	{
		...
		if (tv".$tag_name_x."!='')
		{
			...
			document.getElementById(\"hiddenframe".$tag_name."\").src='/webmin/admin/get_user.php?ID=' + tv".$tag_name_x."+'&strName=".$tag_name."&lang=".LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")."';
		}
		else
...
function IncludeAJAX()
{
	...
	$APPLICATION->AddHeadString('<script src="/webmin/js/main/cphttprequest.js"></script>', true);
}
	...
	function InitJSCore($arExt = array(), $bReturn = false)
	{
		...
		if ($bReturn)
		{
			if (!$arCurrentlyLoadedExt['core'])
				$str .= '<script type="text/javascript" src="/webmin/js/main/core/core.js"></script>'."\r\n";
		}
		else
		{
			$APPLICATION->AddHeadString('<script type="text/javascript" src="/webmin/js/main/core/core.js"></script>', true);
		}
		...
		if ($bReturn)
		{
			if (!$arCurrentlyLoadedExt['core'])
			{
				...
				$str .= '<link href="/webmin/js/main/core/css/core.css" type="text/css" rel="stylesheet" />'."\r\n";
			}
		}
		else
		{
			...
			$APPLICATION->SetAdditionalCSS('/webmin/js/main/core/css/core.css');
		}
		...
		for ($i = 0, $len = count($arExt); $i < $len; $i++)
		{
			...
			if (file_exists($fname))
			{
				$mess_lang_ext = __IncludeLang($fname, true, true);
			}
			...
			if ($bReturn)
			{
				if (!$arCurrentlyLoadedExt[$arExt[$i]])
				{
					...
					$str .= '<script type="text/javascript" src="/webmin/js/main/core/core_'.$arExt[$i].'.js"></script>'."\r\n";

					if (file_exists($_SERVER['DOCUMENT_ROOT'].'/webmin/js/main/core/css/core_'.$arExt[$i].'.css'))
						$str .= '<link href="/webmin/js/main/core/css/core_'.$arExt[$i].'.css" type="text/css" rel="stylesheet" />'."\r\n";
				}
			}
			else
			{
				...
				$APPLICATION->AddHeadString('<script type="text/javascript" src="/webmin/js/main/core/core_'.$arExt[$i].'.js"></script>', true);

				if (file_exists($_SERVER['DOCUMENT_ROOT'].'/webmin/js/main/core/css/core_'.$arExt[$i].'.css'))
					$APPLICATION->SetAdditionalCSS('/webmin/js/main/core/css/core_'.$arExt[$i].'.css');
			}
