...
?>
<script src="/webmin/js/main/dd.js<?echo '?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/webmin/js/main/dd.js')?>" type="text/javascript"></script>
<script>
...
window.structGetSubDir = function(el, div_id, path, dirsonly)
{
	...
	if(div.innerHTML == '')
	{
		...
		CHttpRequest.Send('/webmin/admin/public_structure.php?ajax=Y&<?="lang=".$encLang."&site=".$encSite?>&load_path='+path+(dirsonly? '&dirsonly=Y':''));
	}
...
window.structReload = function(path, params)
{
	...
	CHttpRequest.Send('/webmin/admin/public_structure.php?ajax=Y&reload=Y&<?="lang=".$encLang."&site=".$encSite?>&path='+path+(params? '&'+params:''));
}
...
window.structReloadDirs = function(path)
{
	...
	setTimeout(ShowWaitWindow, 50);
	CHttpRequest.Send('/webmin/admin/public_structure.php?ajax=Y&reload=Y&<?="lang=".$encLang."&site=".$encSite?>&dirsonly=Y&path='+path);
}
...
<?
	$url = $APPLICATION->GetPopupLink(array(
		"URL"=>"/webmin/admin/public_file_new.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&templateID=".$encTemplateID."&path=#PATH#", 
...
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/webmin/admin/public_access_edit.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
		"PARAMS" => Array("min_width"=>450, "min_height" => 250)), "subdialog");
...
<?
	$url = $APPLICATION->GetPopupLink(array(
		"URL"=>"/webmin/admin/public_folder_edit.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
		"PARAMS" => Array("min_width"=>450, "min_height" => 250)), "subdialog");
	$url = str_replace("#PATH#", "'+path+'", $url);
?>
	<?=$url?>;
...
{
//	structShowSubDialog();
//	jsPopup_editor.ShowDialog('/webmin/admin/public_file_edit.php?bxpublic=Y&subdialog=Y&lang=<?=$encLang?>&site=<?=$encSite?>&templateID=<?=$encTemplateID?>&path='+path, {width: 780, height: 570, resize: false});
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/webmin/admin/public_file_edit.php?bxpublic=Y&subdialog=Y&lang=".$encLang."&path=#PATH#&site=".$encSite, 
...
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/webmin/admin/public_file_edit.php?bxpublic=Y&subdialog=Y&lang=".$encLang."&noeditor=Y&path=#PATH#&site=".$encSite, 
		"PARAMS"=>array("width"=>780, "height"=>570, "resize"=>true)), "editor");
	$url = str_replace("#PATH#", "'+path+'", $url);
?>
	<?=$url?>;
...
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/webmin/admin/public_file_property.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
		"PARAMS" => Array("min_width"=>450, "min_height" => 250)), "subdialog");
	$url = str_replace("#PATH#", "'+path+'", $url);
?>
	<?=$url?>;
...
<?
	$url = $APPLICATION->GetPopupLink(array(
		"URL" => "/webmin/admin/public_file_delete.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
...
<?if($bFileman):?>
	...
	items[items.length] = {'SEPARATOR':true};
	items[items.length] = {'TEXT': '<?=CUtil::JSEscape(GetMessage("pub_struct_cp"))?>', 'ONCLICK': 'jsUtils.Redirect(arguments, \'/webmin/admin/fileman_admin.php?lang=<?=$encLang?>&site=<?=$encSite?>&path='+path+'\')', 'TITLE': '<?=CUtil::JSEscape(GetMessage("pub_struct_cp_title"))?>'};
<?endif;?>
...
window.structOpenDirs = function(el)
{
	...
	BX.showWait(strDiv);
	BX.ajax.get(
		'/webmin/admin/public_structure.php?ajax=Y&reload=Y&<?="lang=".$encLang."&site=".$encSite."&path=".$encPath?>&dirsonly=Y', 
		function(result)
		{
