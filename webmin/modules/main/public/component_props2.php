...
{
	require($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/public/component_props.php");
	die();
}
...
if($arComponentDescription["ICON"] <> "" && is_file($_SERVER["DOCUMENT_ROOT"]."/webmin/components".$componentPath.$arComponentDescription["ICON"]))
	$sIcon = "/webmin/components".$componentPath.$arComponentDescription["ICON"];
else
	$sIcon = "/webmin/images/fileman/htmledit2/component.gif";
?>
<?
...
<?endif;?>
<p class="note" title="<?echo GetMessage("comp_prop_path")?>"><a href="/webmin/admin/fileman_admin.php?lang=<?echo LANGUAGE_ID?>&amp;path=<?echo urlencode("/webmin/components".$componentPath)?>"><?echo htmlspecialchars($_GET["component_name"])?></a></p>
<?
if($strWarning <> "")
{
...
$tooltips_path = $_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$cn."/help/.tooltips.php";
$tooltips_lang_path = $_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$cn."/lang/".LANGUAGE_ID."/help/.tooltips.php";
	...
	default: // 'STRING' OR 'FILE' OR 'COLORPICKER' OR 'CUSTOM'
		...
		else
		{
			...
			else
			{
				...
				{
					$src = '/webmin/js/fileman/controls/colorpicker/colorpicker_loader.js';
					$res .= '#COLORPICKER#';
