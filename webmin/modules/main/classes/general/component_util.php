...
class CComponentUtil
{
	...
	function __GetComponentsTree($filterNamespace = False, $arNameFilter = False)
	{
		...
		if ($handle = @opendir($_SERVER["DOCUMENT_ROOT"]."/webmin/components"))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				if (is_dir($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file))
				{
					if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file."/component.php"))
					{
						// It's component
						...
						if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file."/.description.php"))
						{
							CComponentUtil::__IncludeLang("/webmin/components/".$file, ".description.php");

							$arComponentDescription = array();
							$componentName = $file;
							include($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file."/.description.php");
							...
							if (array_key_exists("PATH", $arComponentDescription) && array_key_exists("ID", $arComponentDescription["PATH"]))
							{
								...
								if (array_key_exists("ICON", $arComponentDescription))
									$arComponent["ICON"] = "/webmin/components/".$file.$arComponentDescription["ICON"];
								...
								if (array_key_exists("SCREENSHOT", $arComponentDescription))
								{
									...
									for ($i = 0, $cnt = count($arComponentDescription["SCREENSHOT"]); $i < $cnt; $i++)
										$arComponent["SCREENSHOT"][] = "/webmin/components/".$file.$arComponentDescription["SCREENSHOT"][$i];
								}
					...
					else
					{
						...
						if ($handle1 = @opendir($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file))
						{
							while (($file1 = readdir($handle1)) !== false)
							{
								...
								if (is_dir($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file."/".$file1))
								{
									if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file."/".$file1."/component.php"))
									{
										if ($arNameFilter !== False && !CComponentUtil::CheckComponentName($file1, $arNameFilter))
											continue;
										// It's component
										if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file."/".$file1."/.description.php"))
										{

											CComponentUtil::__IncludeLang("/webmin/components/".$file."/".$file1, ".description.php");

											$arComponentDescription = array();
											$componentName = $file.":".$file1;
											include($_SERVER["DOCUMENT_ROOT"]."/webmin/components/".$file."/".$file1."/.description.php");
											...
											if (array_key_exists("PATH", $arComponentDescription) && array_key_exists("ID", $arComponentDescription["PATH"]))
											{
												...
												$arComponent["DESCRIPTION"] = $arComponentDescription["DESCRIPTION"];
												if (array_key_exists("ICON", $arComponentDescription))
													$arComponent["ICON"] = "/webmin/components/".$file."/".$file1.$arComponentDescription["ICON"];
												if (array_key_exists("COMPLEX", $arComponentDescription) && $arComponentDescription["COMPLEX"] == "Y")
													$arComponent["COMPLEX"] = "Y";
												else
													$arComponent["COMPLEX"] = "N";
												...
												if (array_key_exists("SCREENSHOT", $arComponentDescription))
												{
													...
													for ($i = 0, $cnt = count($arComponentDescription["SCREENSHOT"]); $i < $cnt; $i++)
														$arComponent["SCREENSHOT"][] = "/webmin/components/".$file."/".$file1.$arComponentDescription["SCREENSHOT"][$i];
												}
	...
	function GetComponentDescr($componentName)
	{
		...
		else
		{
			...
			{
				$componentPath = "/webmin/components".$path2Comp;
					...
					foreach ($arSavedParams as $keyTmp => $valueTmp)
					{
						if ($keyTmp == "CACHE_TIME")
						{
							...
							$arComponentParameters["PARAMETERS"]["CACHE_NOTES"] = array(
								"PARENT" => "CACHE_SETTINGS",
								"TYPE" => "CUSTOM",
								"JS_FILE" => "/webmin/js/main/comp_props.js",
	...
	function GetTemplateProps($componentName, $templateName, $siteTemplate = "", $arCurrentValues = array())
	{
		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/components".$path2Comp."/templates/".$templateName))
		{
			if (is_dir($_SERVER["DOCUMENT_ROOT"]."/webmin/components".$path2Comp."/templates/".$templateName)
				&& file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/components".$path2Comp."/templates/".$templateName."/.parameters.php"))
			{
				CComponentUtil::__IncludeLang("/webmin/components".$path2Comp."/templates/".$templateName, ".parameters.php");
				include($_SERVER["DOCUMENT_ROOT"]."/webmin/components".$path2Comp."/templates/".$templateName."/.parameters.php");
			}
			return $arTemplateParameters;
		}
	...
	function CopyComponent($componentName, $newNamespace, $newName = False, $bRewrite = False)
	{
		...
		$componentPathNew = "/webmin/components".$path2CompNew;
	...
	function CopyTemplate($componentName, $templateName, $siteTemplate, $newSiteTemplate, $newName = False, $bRewrite = False)
	{
		...
		if ($siteTemplate != False)
			...
			$path = "/webmin/components".$path2Comp."/templates/".$templateName;
