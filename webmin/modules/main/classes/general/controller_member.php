...
class CControllerClient
{
	...
	{
		return file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/update_db_updater.php");
	}
...
class __CControllerPacket
{
	...
	function Debug($sText)
	{
		...
		if(!defined($this->debug_file_const))
			$dirname = $_SERVER['DOCUMENT_ROOT'].'/webmin/controller_logs/';
		else
...
class CControllerClientRequestTo extends __CControllerPacketRequest
{
	...
	function SendWithCheck($page="/webmin/admin/controller_ws.php")
	{
	...
	function Send($page="/webmin/admin/controller_ws.php")
	{
		$this->Sign();
...
class CControllerTools
{
	...
	function UnpackFileArchive($strfile, $path_to)
	{
		...
		include_once(dirname(__FILE__) . '/tar_gz.php');
		$arcname = '/webmin/tmp/controller/' . md5(uniqid(rand(), true)) . '.tar.gz';
		CheckDirPath($_SERVER['DOCUMENT_ROOT'] . $arcname);
