...
class CAllUserTypeManager
{
	...
	*/
	...
	function EditFormShowTab($entity_id, $bVarsFromForm, $ID)
	{
		...
		{
			echo "<tr colspan=\"2\"><td align=\"left\"><a href=\"/webmin/admin/userfield_edit.php?lang=".LANG."&ENTITY_ID=".urlencode($entity_id)."&back_url=".urlencode($APPLICATION->GetCurPageParam()."&tabControl_active_tab=user_fields_tab")."\">".GetMessage("USER_TYPE_EDIT_TAB_HREF")."</a></td></tr>";
		}
	...
	function GetEditFormHTML($bVarsFromForm, $form_value, $arUserField)
	{
		...
		if($arUserField["USER_TYPE"])
		{
			if($this->GetRights($arUserField["ENTITY_ID"]) >= "W")
				$strLabelHTML = '<a href="/webmin/admin/userfield_edit.php?lang='.LANG.'&ID='.urlencode($arUserField["ID"]).'&back_url='.urlencode($APPLICATION->GetCurPageParam().'&tabControl_active_tab=user_fields_tab').'">'.htmlspecialchars($arUserField["EDIT_FORM_LABEL"]? $arUserField["EDIT_FORM_LABEL"]: $arUserField["FIELD_NAME"]).'</a>'.($arUserField["MANDATORY"]=="Y"? '<span class="required">*</span>': '').':';
			else
