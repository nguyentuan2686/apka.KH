...
class CMenu
{
	...
	function GetMenuHtmlEx()
	{
		...
		if($APPLICATION->GetShowIncludeAreas())
		{
			...
			if($menu_perm>="W")
				$arIcons[] = Array(
					"URL"=>"/webmin/admin/fileman_menu_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&path=".urlencode($this->MenuDir)."&name=".$this->type,
					"ICON"=>"menu-edit",
					"TITLE"=>GetMessage("MAIN_MENU_EDIT")
				);
			if($templ_perm>="W" && $USER->IsAdmin())
				$arIcons[] = Array(
					"URL"=>"/webmin/admin/fileman_file_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&full_src=Y&path=".urlencode($this->template),
					"ICON"=>"menu-template",
