...
class CAllMain
{
	...
	var $ShowPanel = NULL, $PanelShowed = false, $bPanelIcons = false, $bPanelScriptsIncluded = false;
	...
	function AuthForm($mess, $show_prolog=true, $show_epilog=true, $not_show_links="N", $do_die=true)
	{
		...
		if($isAdmin == "")
		{
			...
			{
				$this->IncludeComponent("webmin:".$comp_name, "", array(
	...
	function GetCSS($cMaxStylesCnt=20, $bXhtmlStyle=true)
	{
		...
		foreach($arCSS as $css_path)
		{
			...
			if(($cnt<$cMaxStylesCnt || $bLink || $ind > count($arCSS)-2) && strncmp($css_path, '/webmin/modules/', 16) != 0)
			{
	...
	{
		//echo '<pre>'; print_r($arIcons); echo '</pre>';
		...
		foreach ($arIcons as $arIcon)
		{
			...
			else
			{
				...
				if ($arIcon['MENU'])
				{
					foreach ($arIcon['MENU'] as $k => $aMenuItem)
					{
						$u = $aMenuItem['URL'];
						...
						$arIcon['MENU'][$k]['ONCLICK'] = $u;
					}
					
					$jsIcon['MENU'] = $arIcon['MENU'];
				}
		...
		if ($this->includeLevel > ($GLOBALS['BX_GLOBAL_AREA_EDIT_ICON'] ? 1 : 0))
		{
			return '';
		}
		else
		{
			...
			//echo '<pre>'; print_r($this->includeAreaIcons); echo '</pre>';
			foreach ($this->includeAreaIcons as $arSubIcons)
			{
				...
				$arAllInnerIcons = array_merge(
					...
					array(
						array(
							'TEXT' => $arSubIcons['DESCRIPTION']['NAME'].' ('.$arSubIcons['COMPONENT'].')', 
							'ICON' => 'parameters-2',
			...
			//echo '<pre>'; print_r($arAllOuterIcons); echo '</pre>';
			...
			$arJSIcons = array_merge(
				...
				array(
						array(
							...
							'ICON' => 'parameters-2',
							'MENU' => array_merge($arAddInnerIcons, $arAllInnerIcons)
						)
				)
			);
			...
			if ($arParams['COMPONENT_ID'])
			{
				...
				if (is_array($arUserOptions) && $arUserOptions['pin'] === 'true')
				{
					$arJSParams['pin'] = true;
					if ($arUserOptions['transform']) 
						$arJSParams['transform'] = ($arUserOptions['transform'] === 'true');
					if ($arUserOptions['top']) $arJSParams['top'] = $arUserOptions['top'];
					if ($arUserOptions['left']) $arJSParams['left'] = $arUserOptions['left'];
				}
			}
			...
			$res = '<script type="text/javascript">BX.ready(function() {(new BX.CMenuOpener('.CUtil::PhpToJsObject($arJSParams).')).Show()});</script>';
}
/*
		$res .= '
...
*/
	...
	function GetTemplatePath($rel_path)
	{
		if(substr($rel_path, 0, 1)!="/")
		{
			...
			{
				$path = "/webmin/modules/".$module_id."/install/templates/".$rel_path;
				if(file_exists($_SERVER["DOCUMENT_ROOT"].$path))
					return $path;
			}
	...
	function IncludeComponent($componentName, $componentTemplate, $arParams = array(), $parentComponent = null, $arFunctionParams = array())
	{
			...
			&& ($APPLICATION->GetShowIncludeAreas()))
		{
			if(function_exists("debug_backtrace"))
			{
				...
				if($iSrcLine > 0 && $sSrcFile <> "")
				{
					...
					{
						//outside
						$sRealBitrix = strtolower(str_replace("\\", "/", realpath($_SERVER["DOCUMENT_ROOT"]."/webmin")));
						if(strpos(strtolower($sSrcFile), $sRealBitrix) === 0)
						{
							$sSrcFile = "/webmin".substr($sSrcFile, strlen($sRealBitrix));
							$bSrcFound = true;
						}
					}
				}
		...
		if($_SESSION["SESS_SHOW_INCLUDE_TIME_EXEC"]=="Y" && ($USER->CanDoOperation('edit_php') || $_SESSION["SHOW_SQL_STAT"]=="Y"))
			echo $debug->Output($componentName, "/webmin/components".$componentRelativePath."/component.php", $arParams["CACHE_TYPE"].$arParams["MENU_CACHE_TYPE"]);
		elseif($APPLICATION->ShowIncludeStat)
			$debug->Stop($componentName, "/webmin/components".$componentRelativePath."/component.php", $arParams["CACHE_TYPE"].$arParams["MENU_CACHE_TYPE"]);
		...
		if($bDrawIcons)
		{
			...
			if($bComponentAccess && !$parentComponent && $bSrcFound /* && $showMode == "configure"*/)
			{
				$url = $APPLICATION->GetPopupLink(
					array(
						'URL' => "/webmin/admin/component_props.php?".
				...
				$arIcons[] = array(
					...
					'ALT'=>GetMessage("main_incl_file_comp_param"),
					'DEFAULT'=>true,
				);
			...
			if($bComponentAccess /*&& $showMode == "configure"*/)
			{
				...
				if(!is_null($template))
				{
					...
					if($bSrcFound && $template->IsInTheme() == false)
					{
						...
						$urlCopy = "/webmin/admin/template_copy.php?".
							"lang=".urlencode(CUtil::addslashes(LANGUAGE_ID)).
					...
					if($USER->CanDoOperation('edit_php') && strlen($template->GetSiteTemplate()) > 0)
					{
						...
						$arIcons[] = array(
							'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
									'URL' => "/webmin/admin/public_file_edit_src.php?site=".SITE_ID."&".'path='.urlencode($template->GetFile())."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
									'PARAMS' => array(
										'width' => 770,
										'height' => 570,
										'resize' => true
						...
						if(StrLen($template->GetFolder()) > 0)
						{
							if(file_exists($_SERVER["DOCUMENT_ROOT"].$template->GetFolder()."/style.css"))
							{
								...
								$arIcons[] = array(
									'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
											'URL' => "/webmin/admin/public_file_edit_src.php?site=".SITE_ID."&".'path='.urlencode($template->GetFolder()."/style.css")."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
											'PARAMS' => array(
												'width' => 770,
												'height' => 570,
												'resize' => true
			...
			{
				$arPanelParams['COMPONENT'] = $componentName;
					...
					foreach($arComponentDescription["AREA_BUTTONS"] as $value)
					{
						...
						*/
						...
						if (array_key_exists("SRC", $value))
							$value["SRC"] = "/webmin/components".$componentRelativePath.$value["SRC"];
						$aAddIcons[] = $value;
					}
				}
			}
			...
			*/
			...
			if ($bDrawIcons)
			{
				if ($bSrcFound)
				{
					$arPanelParams['COMPONENT_ID'] = md5($arPanelParams['COMPONENT'].'|'.$sSrcFile.':'.$iSrcLine);
				}
	...
	{
		require_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/php_parser.php");
	...
	function IncludeFile($rel_path, $arParams = Array(), $arFunctionParams = Array())
	{
		...
		if($bDrawIcons)
		{
			...
			if ($sType != 'TEMPLATE')
			{
				...
				{
					case 'html':
						$editor = "/webmin/admin/fileman_html_edit.php?site=".SITE_ID."&";
					break;
					case 'text':
						$editor = "/webmin/admin/fileman_file_edit.php?site=".SITE_ID."&";
					break;
					case 'php':
						$editor = "/webmin/admin/fileman_file_edit.php?full_src=Y&site=".SITE_ID."&";
					break;
				}
			}
			else
			{
				...
				{
					case 'html':
						$editor = '/webmin/admin/public_file_edit.php?bxpublic=Y&from=includefile&';
						$resize = 'false';
					break;

					case 'text':
						$editor = '/webmin/admin/public_file_edit.php?bxpublic=Y&from=includefile&noeditor=Y&';
						$resize = 'true';
					break;

					case 'php':
						$editor = '/webmin/admin/public_file_edit_src.php?';
						$resize = 'true';
					break;
				}
			}
			...
			else
			{
				...
				{
					$arIcons[] = Array(
						'URL'=>'/webmin/admin/workflow_edit.php?'.$arFunctionParams["LANG"].'&fname='.urlencode($path).$arFunctionParams["TEMPLATE"].$arFunctionParams["BACK_URL"],
						'ICON'=>'bx-context-toolbar-edit-icon',
		...
		if($APPLICATION->GetShowIncludeAreas() && $USER->CanDoOperation('edit_php') && $bShowIcons)
		{
			...
			if(strpos($strChainTemplate, $DOC_ROOT)===0)
			{
				...
				if((!defined("ADMIN_SECTION") || ADMIN_SECTION!==true) && $templ_perm>="W")
				{
					$arIcons = Array();
					$arIcons[] = Array(
						"URL"=>"/webmin/admin/fileman_file_edit.php?lang=".LANGUAGE_ID."&site=".$site."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&full_src=Y&path=".urlencode($path),
						"ICON"=>"nav-template",
	...
	*/
			...
			if(is_array($this->arrSPREAD_COOKIE) && count($this->arrSPREAD_COOKIE)>0)
			{
				...
				if(count($arrDomain)>0)
				{
					...
					foreach($arUniqDomains as $domain)
					{
						...
						{
							$url = $protocol.$domain."/webmin/spread.php?".$params;
	...
	function __GetPanel()
	{
		...
		if(isset($USER) && is_object($USER) && $USER->IsAuthorized() && (!array_key_exists("bx_hit_hash", $_REQUEST) || strlen($_REQUEST["bx_hit_hash"]) <= 0))
		{
			return GetPanelHtml();
		}
	}
	...
	function ShowPanel()
	{
		AddEventHandler('main', 'OnBeforeEndBufferContent', array($this, 'ShowPanelScripts'));
		$this->AddBufferContent(Array(&$this, "__GetPanel"));
	}

	function __ShowPanelScripts($bReturn = false)
	{
		if ($this->bPanelScriptsIncluded) return;
		$this->bPanelScriptsIncluded = true;
		
		if (!$bReturn)
		{
			CUtil::InitJSCore(array('window', 'ajax', 'admin'));
			$this->AddHeadString($GLOBALS["adminPage"]->ShowScript());
			$this->AddHeadScript('/webmin/js/main/public_tools.js');
			$this->SetAdditionalCSS(ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/pubstyles.css');
			//$this->SetAdditionalCSS(ADMIN_THEMES_PATH.'/'.ADMIN_THEME_ID.'/jspopup.css');
		}
		else
		{
			return 
				...
				.'
<script type="text/javascript" src="/webmin/js/main/public_tools.js'.'?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/webmin/js/main/public_tools.js').'"></script>
'
...
'
	...
	{
		if ($this->bPanelIcons) return;
		$this->bPanelIcons = true;
	...
	function ShowPanelScripts()
	{
		if ($this->ShowPanel === false || (!$GLOBALS['USER']->IsAuthorized() && $this->ShowPanel !== true))
			return;
		
		$this->InitPanelIcons();
	
		$bShowPanel = false;
		foreach($this->arPanelButtons as $key=>$arValue)
		{
	...
	function ConvertCharset($string, $charset_in, $charset_out)
	{
		...
		{
			include_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/charset_converter.php");
	...
	{
		include_once($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/classes/general/captcha.php");
