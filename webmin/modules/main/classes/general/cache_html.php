...
/*. float .*/
	...
	function OnEpilog()
	{
		...
		$bExcludeByFile = $_SERVER["SCRIPT_NAME"] == "/webmin/admin/get_start_menu.php";
		...
		if(($_SERVER["REQUEST_METHOD"] === "POST") || ($bAutorized && check_bitrix_sessid() && !$bExcludeByFile))
		{
			...
			if(strncmp($_SERVER["REQUEST_URI"], "/webmin/", 8) === 0)
			{
	...
	function GetOptions($bCheck = false)
	{
		...
		{
			$arHTMLPagesOptions["EXCLUDE_MASK"] = "/webmin/*;/404.php";
			$bCompile = true;
		}
