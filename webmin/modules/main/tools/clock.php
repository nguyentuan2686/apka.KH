...
class CClock
{
	...
	function Show($arParams)
	{
		...
		{
			?><a href="javascript:void(0);" onclick="bxShowClock_<?=$arParams['inputId']?>()" title="<?=GetMessage('BX_CLOCK_TITLE')?>" onmouseover="this.className='bxc-icon-hover';" onmouseout="this.className='';"><img id="<?=$arParams['inputId']?>_icon" src="/webmin/images/1.gif" class="bx-clock-icon bxc-iconkit-c"></a><?
		}
		...
		function bxc_load_css()
		{
			if (!window.BXClockStyles)
				window.BXClockStyles = jsUtils.loadCSSFile(['/webmin/themes/.default/clock.css']);
		}
		if (!window.phpVars)
		...
		if (!window.jsUtils)
		{
			setTimeout(function(){
				...
				oSript.src = '/webmin/js/main/utils.js';
		...
		{
			if (!window.JCClock)
				return jsUtils.loadJSFile(['/webmin/js/main/clock.js'], bxShowClock_<?=$arParams['inputId']?>);
