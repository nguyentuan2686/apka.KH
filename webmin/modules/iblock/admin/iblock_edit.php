			...
			if(!$bVarsFromForm)
			{
				...
				if(strlen($apply)<=0)
				{
					if(strlen($_REQUEST["return_url"])>0)
						...
						LocalRedirect("/webmin/admin/iblock_admin.php?type=".$type."&lang=".LANG."&admin=".($_REQUEST["admin"]=="Y"? "Y": "N"));
				}
				LocalRedirect("/webmin/admin/iblock_edit.php?type=".$type."&tabControl_active_tab=".urlencode($tabControl_active_tab)."&lang=".LANG."&ID=".$ID."&admin=".($_REQUEST["admin"]=="Y"? "Y": "N").(strlen($_REQUEST["return_url"])>0? "&return_url=".urlencode($_REQUEST["return_url"]): ""));
			}
		}
	}
...
?>
	...
	<tr>
		<td valign="top" nowrap width="40%"><?echo GetMessage("IB_E_EVERYONE")?> [<a class="tablebodylink" href="/webmin/admin/group_edit.php?ID=2&amp;lang=<?=LANGUAGE_ID?>">2</a>]:</td>
		<td valign="top" width="60%">
	...
	while($r = $groups->GetNext()):
		...
		?>
	<tr>
		<td valign="top" nowrap width="40%"><?echo $r["NAME"]?> [<a class="tablebodylink" href="/webmin/admin/group_edit.php?ID=<?=$r["ID"]?>&lang=<?=LANGUAGE_ID?>"><?=$r["ID"]?></a>]:</td>
		<td valign="top" width="60%">
...
$tabControl->BeginNextTab();
	...
	<tr>
		<td valign="top" colspan="2">
						...
						<tr>
							<td valign="top">
								<?if(IsModuleInstalled("bizprocdesigner")):?>
									<a href="/webmin/admin/iblock_bizproc_workflow_edit.php?document_type=iblock_<?= $ID ?>&lang=<?=LANGUAGE_ID?>&ID=<?=$arTemplate["ID"]?>&back_url_list=<?= urlencode($APPLICATION->GetCurPageParam("", array()))?>" target="_blank"><?= $arTemplate["NAME"] ?> [<?=$arTemplate["ID"]?>]</a>
								<?else:?>
									<?= $arTemplate["NAME"] ?>
								<?endif?>
			...
			<?endif;?>
			<?if(IsModuleInstalled("bizprocdesigner")):?>
			<a href="/webmin/admin/iblock_bizproc_workflow_admin.php?document_type=iblock_<?= $ID ?>&lang=<?=LANGUAGE_ID?>&back_url_list=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>" target="_blank"><?echo GetMessage("IB_E_GOTO_BP")?></a>
			<?endif?>
		</td>
	</tr>
	<?
