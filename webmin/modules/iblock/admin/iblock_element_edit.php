...
function _ShowElementPropertyField($name, $property_fields, $values, $bVarsFromForm = false)
{
	...
	foreach($values as $key=>$val)
	{
		...
		echo '<tr><td>'.
			...
			'<input type="button" value="..." onClick="jsUtils.OpenWindow(\'/webmin/admin/iblock_element_search.php?lang='.LANG.'&amp;IBLOCK_ID='.$property_fields["LINK_IBLOCK_ID"].'&amp;n='.$name.'&amp;k='.$key.'\', 600, 500);">'.
	...
	{
		echo '<tr><td>'.
			'<input type="button" value="'.GetMessage("IBLOCK_ELEMENT_EDIT_PROP_ADD").'..." onClick="jsUtils.OpenWindow(\'/webmin/admin/iblock_element_search.php?lang='.LANG.'&amp;IBLOCK_ID='.$property_fields["LINK_IBLOCK_ID"].'&amp;n='.$name.'&amp;m=y&amp;k='.$key.'\', 600, 500);">'.
			'<span id="sp_'.md5($name).'_'.$key.'" ></span>'.
			'</td></tr>';
	}
	...
	echo "	oCell.innerHTML=".
		...
		"'onClick=\"jsUtils.OpenWindow(\'/webmin/admin/iblock_element_search.php?lang=".LANG."&amp;IBLOCK_ID=".$property_fields["LINK_IBLOCK_ID"]."&amp;n=".$name."&amp;k='+MV_".md5($name)."+'\', '+\r\n".
		"' 600, 500);\">'+".
	...
	if($historyId <= 0 && $REQUEST_METHOD=="POST" && strlen($Update)>0 && $view!="Y" && (!$error) && empty($dontsave))
	{
		...
		else
		{
			...
			else
			{
				...
				if (
					...
					&& file_exists($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/catalog/admin/templates/product_edit_validator.php")
				)
				{
					$bCatalog = true;
					// errors'll be appended to $strWarning;
					include($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/catalog/admin/templates/product_edit_validator.php");
				}
				...
				if (strlen($strWarning) <= 0)
				{
					...
					{
						include($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/catalog/admin/templates/product_edit_action.php");
					}
				} // if ($strWarning)
		...
		else
		{
			...
			if(strlen($apply) <= 0)
			{
				...
				{
					LocalRedirect("/webmin/admin/".CIBlock::GetAdminElementListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section))));
				}
			}
			else
			{
				LocalRedirect("/webmin/admin/iblock_element_edit.php?ID=".$ID.($WF=="Y"?"&WF=Y":"")."&lang=".LANG. "&type=".htmlspecialchars($type)."&".$tabControl->ActiveTabParam()."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section).(strlen($return_url)>0?"&return_url=".UrlEncode($return_url):""));
			}
		}
	}
...
<?
	...
	if($arTranslit["TRANSLITERATION"] == "Y")
	{
		...
		function set_linked()
		{
			...
			{
				if(linked)
					name_link.src='/webmin/themes/.default/icons/iblock/link.gif';
				else
					name_link.src='/webmin/themes/.default/icons/iblock/unlink.gif';
			}
			var code_link = document.getElementById('code_link');
			if(code_link)
			{
				if(linked)
					code_link.src='/webmin/themes/.default/icons/iblock/link.gif';
				else
					code_link.src='/webmin/themes/.default/icons/iblock/unlink.gif';
			}
...
$tabControl->Begin(array(
	"FORM_ACTION" => "/webmin/admin/iblock_element_edit.php?type=".urlencode($type)."&lang=".LANG."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section),
));
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	?>
		<tr id="tr_NAME">
			...
			<td nowrap>
				<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/webmin/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
	...
	?>
		<tr id="tr_CODE">
			...
			<td nowrap>
				...
				<input type="text" size="50" name="CODE" id="CODE" maxlength="255" value="<?echo $str_CODE?>"><image id="code_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/webmin/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
...
if(count($PROP)>0):
	...
	if ($view!="Y" && CModule::IncludeModule("catalog") && CCatalog::GetByID($IBLOCK_ID))
	{
		...
		include($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/catalog/admin/templates/product_edit.php");
	...
	<?else:?>
		...
		<td>
			...
			<tr>
				...
				<input type="button" value="<?echo GetMessage("IBLOCK_ELEMENT_EDIT_PROP_ADD")?>..." onClick="jsUtils.OpenWindow('/webmin/admin/iblock_section_search.php?lang=<?echo LANG?>&amp;IBLOCK_ID=<?echo $IBLOCK_ID?>&amp;n=input_IBLOCK_SECTION&amp;m=y', 600, 500);">
				</td>
				<td>&nbsp;</td>
			</tr>
			</table>
