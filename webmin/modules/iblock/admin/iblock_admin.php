...
if($arIBTYPE===false)
	LocalRedirect("/webmin/admin/iblock_type_admin.php?lang=".LANG);
...
while($dbrs = $rsIBlocks->NavNext(true, "f_"))
{
	...
	if($dbrs["BIZPROC"] == "Y" && $bBizproc && in_array("WORKFLOW_TEMPLATES", $lAdmin->GetVisibleHeaderColumns()))
	{
		if(IsModuleInstalled("bizprocdesigner"))
		{
			...
			$row->AddViewField("WORKFLOW_TEMPLATES", '<a href="/webmin/admin/iblock_bizproc_workflow_admin.php?document_type=iblock_'.$f_ID.'&lang='.LANGUAGE_ID.'&back_url_list='.urlencode($APPLICATION->GetCurPageParam("", array())).'">'.$cnt.'</a>');
		}
	}
	...
	if($dbrs["BIZPROC"] == "Y" && CIBlock::GetPermission($f_ID) >= "X" && IsModuleInstalled("bizproc") && IsModuleInstalled("bizprocdesigner"))
	{
		$arActions[] = array(
			...
			"ACTION"=>"window.location='/webmin/admin/iblock_bizproc_workflow_admin.php?document_type=iblock_".$f_ID."&lang=".LANGUAGE_ID."';"
			);
	}
