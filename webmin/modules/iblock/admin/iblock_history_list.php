...
if(!$zr=$z->Fetch())
{
	$APPLICATION->SetTitle(GetMessage("IBLOCK_ADM_HISTORY_TITLE", array("#ID#" => $ELEMENT_ID)));
	require($_SERVER["DOCUMENT_ROOT"]."/webmin/modules/main/include/prolog_admin_after.php");
	?><a href="/webmin/admin/iblock_admin.php?type=<?echo htmlspecialchars($type)?>&amp;lang=<?echo LANG?>"><?echo htmlspecialcharsex($arIBTYPE["NAME"])?></a> - <a href="<?echo htmlspecialchars(CIBlock::GetAdminElementListLink($IBLOCK_ID, array()))?>"><?echo htmlspecialchars($arIBlock["NAME"])?></a><?
