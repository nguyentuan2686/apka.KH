...
if($REQUEST_METHOD=="POST" && strlen($Update)>0 && check_bitrix_sessid())
{
	...
	else
	{
		...
		if(strlen($apply)<=0)
		{
			...
			{
				LocalRedirect("/webmin/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section))));
			}
		}

		LocalRedirect("/webmin/admin/iblock_section_edit.php?lang=". LANG."&type=".urlencode($type)."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section)."&".$tabControl->ActiveTabParam().'&ID='.$ID.(strlen($return_url)>0?"&return_url=".UrlEncode($return_url):""));
	}
}
...
$tabControl->Begin(array(
	"FORM_ACTION" => "/webmin/admin/iblock_section_edit.php?type=".urlencode($type)."&lang=".LANG."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section),
));
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	?>
		<tr id="tr_NAME">
			<td><?echo $tabControl->GetCustomLabelHTML()?></td>
			<td nowrap>
				<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="name_link" title="<?echo GetMessage("IBSEC_E_LINK_TIP")?>" class="linked" src="/webmin/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
	...
	?>
		<tr id="tr_CODE">
			...
			<td nowrap>
				...
				<input type="text" size="50" name="CODE" id="CODE" maxlength="255" value="<?echo $str_CODE?>"><image id="code_link" title="<?echo GetMessage("IBSEC_E_LINK_TIP")?>" class="linked" src="/webmin/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
...
)
{
	...
	if($USER_FIELD_MANAGER->GetRights($entity_id) >= "W")
	{
		...
		?>
			<tr colspan="2">
				<td align="left">
					<a href="/webmin/admin/userfield_edit.php?lang=<?echo LANG?>&amp;ENTITY_ID=<?echo urlencode($entity_id)?>&amp;back_url=<?echo urlencode($APPLICATION->GetCurPageParam()."&tabControl_active_tab=user_fields_tab")?>"><?echo $tabControl->GetCustomLabelHTML()?></a>
				</td>
			</tr>
		<?
...
if(strlen($return_url)>0)
	...
	$bu = "/webmin/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section)));
