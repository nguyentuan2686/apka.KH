<?
if($GLOBALS["USER"]->GetID() > 0){
    LocalRedirect('/app.book/app/');
}
?>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>APKIN | Log in</title>
    <?$APPLICATION->ShowHead();?>
    <!-- Tell the browser to be responsive to screen width -->
    <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
    <!-- Bootstrap 3.3.5 -->
    <link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/bootstrap/css/bootstrap.min.css">
    <!-- Font Awesome -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css">
    <!-- Ionicons -->
    <link rel="stylesheet" href="https://code.ionicframework.com/ionicons/2.0.1/css/ionicons.min.css">
    <!-- Theme style -->
    <link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/dist/css/AdminLTE.min.css">
    <!-- iCheck -->
    <link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/plugins/iCheck/square/blue.css">
     
    <!-- jQuery 2.1.4 -->
    <script src="<?=SITE_TEMPLATE_PATH?>/plugins/jQuery/jQuery-2.1.4.min.js"></script>
    <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
        <script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
        <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->
  </head>
  <body class="hold-transition login-page">
   <?$APPLICATION->IncludeComponent(
	"webmin:system.auth.form",
	"apkin",
	Array(
		"REGISTER_URL" => "",
		"PROFILE_URL" => "",
		"SHOW_ERRORS" => "Y"
	),
false
);?>


<?$APPLICATION->IncludeComponent("webmin:main.register", "login.page", array(
	"SHOW_FIELDS" => array(
		0 => "NAME",
		1 => "PERSONAL_PHONE"
	),
	"REQUIRED_FIELDS" => array(
    0 => "NAME",
    1 => "PERSONAL_PHONE"
	),
	"SEF_MODE" => "N",
	"SEF_FOLDER" => "/",
	"AUTH" => "N",
	"USE_BACKURL" => "Y",
	"SUCCESS_PAGE" => "",
	"SET_TITLE" => "Y",
	"USER_PROPERTY" => array(
		0 => "UF_VI_TAI_KHOAN"

	),
	"USER_PROPERTY_NAME" => "",
    "ACTIVE_USER" => "Y"
	),
	false
);?>
    <!-- Bootstrap 3.3.5 -->
    <script src="<?=SITE_TEMPLATE_PATH?>/bootstrap/js/bootstrap.min.js"></script>
    <!-- iCheck -->
    <script src="<?=SITE_TEMPLATE_PATH?>/plugins/iCheck/icheck.min.js"></script>
    <script>
      $(function () {
        $('input').iCheck({
          checkboxClass: 'icheckbox_square-blue',
          radioClass: 'iradio_square-blue',
          increaseArea: '20%' // optional
        });
      });
    </script>
  </body>
</html>
<?die();?>