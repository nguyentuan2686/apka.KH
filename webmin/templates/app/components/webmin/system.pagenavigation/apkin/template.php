...
<?if($arResult["bDescPageNumbering"] === true):?>
	...
	<div class="nav-title">Page <?=$arResult["NavPageNomer"]?> <?=GetMessage("nav_of")?> <?=$arResult["NavRecordCount"]?></div>
...
<?else:?>
	...
	<?while($arResult["nStartPage"] <= $arResult["nEndPage"]):?>
		...
		<?elseif($arResult["nStartPage"] == 1 && $arResult["bSavePage"] == false):?>
			<a href="<?=$arResult["sUrlPath"]?><?=$strNavQueryStringFull?>" rel="nofollow"><?=$arResult["nStartPage"]?></a>&nbsp;
		<?else:?>
			<a href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=$arResult["nStartPage"]?>" rel="nofollow"><?=$arResult["nStartPage"]?></a>&nbsp;
		<?endif?>
		<?$arResult["nStartPage"]++?>
	<?endwhile?>
